...
function getSummaryMessage(group: UserGroup | "unknown"): string {
  ...
  if (group === "franchise") {
    return "AI탭(베타 · 음식점·카페 등 위주, 6월 전체 확대 예정)과 글로벌 AI(ChatGPT·Gemini) 2개 채널에서 노출 가능합니다.";
  }
  if (group === "LIKELY") {
    return "현재 AI 탭과 글로벌 AI 2개 채널에서 노출 가능하며, AI 브리핑은 곧 확대 예정입니다.";
  }
  if (group === "INACTIVE") {
    return "AI탭(베타 · 음식점·카페 등 위주, 6월 전체 확대 예정)·ChatGPT·Gemini 핵심 채널을 모두 측정합니다.";
  }
    ...
    <div className={`${isLanding ? "rounded-2xl border border-blue-100 bg-white p-5 md:p-7 shadow-sm" : "rounded-xl"}`}>
      ...
      <div className={wrapClass}>
        ...
        <ChannelCard
          ...
          badge="참여 가능"
          badgeVariant="all"
          description="검색 'AI' 탭. 현재 음식점·카페 등 장소형 업종 위주 (2026-06 전체 확대 예정)"
          disclaimer="베타 · 음식점·카페 등 위주 · 콘텐츠 품질에 따라 다름"
          highlight={group !== "ACTIVE"}
          compact={isCompact}
        />

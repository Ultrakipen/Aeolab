...
export function IneligibleBusinessNotice({ categoryLabel, eligibility, isFranchise = false }: Props) {
  ...
  const title = isFranchise
    ...
    : isInactive
    ? `${categoryLabel} 업종 — AI탭(베타 · 6월 전체 확대 예정) + ChatGPT·Gemini 장기 최적화`
    : `${categoryLabel} 업종 — AI 브리핑 확대 예정 + 글로벌 AI 최적화 병행`;

  const description = isFranchise
    ? "네이버 AI탭(베타 · 음식점·카페 등 장소형 위주, 2026-06 전체 확대 예정)과 ChatGPT·Gemini·Google AI 노출을 집중 측정·개선합니다. AI 브리핑은 가맹점 정책에 따라 향후 확대 예정입니다."
    : isInactive
    ? `${categoryLabel} 업종도 네이버 AI탭(2026-04-27 베타, 현재 음식점·카페 등 장소형 위주 — 2026-06 전체 확대 예정)·ChatGPT·Gemini를 통해 AI 검색에 노출될 수 있습니다. AEOlab이 5채널 노출 현황을 자동 측정하고 개선 방향을 제시합니다.`
    : "네이버 AI탭(베타 · 음식점·카페 등 위주, 6월 전체 확대 예정)·ChatGPT·Gemini를 지금 바로 최적화할 수 있습니다. AI 브리핑은 업종 확대 시 즉시 활성화됩니다.";

  const channels = isInactive || isFranchise
    ? [
        { icon: "✅", text: "네이버 AI 탭 — 2026-04-27 베타 공개, 현재 음식점·카페 등 위주 · 2026-06 전체 사용자 확대 예정" },

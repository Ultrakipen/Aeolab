 ...
 * - AI탭: 모든 업종 가능, 2026-04-27 베타, 2026-06 전체 확대 (공식 발표)
 *
 * 자기 업종 기준으로 "현재 대상" 배지를 자동 표시.
 */
export default function NaverAiPathwayCard({ briefingEligibility, isFranchise, latestAdOnly, globalWeight, category }: Props) {
  ...
  // 업종별 최우선 채널 추천 (docs/ai_channel_exposure_criteria_v1.0.md §업종별 최우선 채널 가이드)
  const getChannelRecommendation = (): { label: string; channels: string; color: string } => {
    if (isFranchise) {
      return { label: "프랜차이즈 가맹점", channels: "AI탭 + ChatGPT + Gemini", color: "bg-amber-50 border-amber-200 text-amber-900" };
    }
    if (briefingEligibility === "active") {
      if (category === "accommodation") {
        return { label: "내 업종", channels: "AI 브리핑 + AI탭 + Gemini", color: "bg-blue-50 border-blue-200 text-blue-900" };
      }
      return { label: "내 업종", channels: "AI 브리핑 + AI탭", color: "bg-blue-50 border-blue-200 text-blue-900" };
    }
    if (briefingEligibility === "likely") {
      return { label: "내 업종", channels: "AI탭 + ChatGPT + Gemini", color: "bg-amber-50 border-amber-200 text-amber-900" };
    }
    // INACTIVE (AI 브리핑 비대상이지만 AI탭은 업종 무관 노출 가능 — 공식 게이팅 없음)
    if (category === "shopping" || category === "fashion") {
      return { label: "내 업종", channels: "AI탭(쇼핑 에이전트) + Gemini + ChatGPT", color: "bg-purple-50 border-purple-200 text-purple-900" };
    }
    return { label: "내 업종", channels: "AI탭 + ChatGPT + Gemini", color: "bg-purple-50 border-purple-200 text-purple-900" };
  };
  const channelRec = getChannelRecommendation();
  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-6">
        ...
        {/* 내 업종 최우선 채널 추천 — 업종별로 어느 채널에 집중할지 명확 안내 */}
        <div className={`rounded-lg border px-3 py-2.5 mb-4 flex items-start gap-2 ${channelRec.color}`}>
          <span className="text-base shrink-0">🎯</span>
          <p className="text-sm md:text-base leading-snug break-keep">
            <strong>{channelRec.label} 최우선 채널:</strong>{" "}
            <span className="font-bold">{channelRec.channels}</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          ...
          >
            ...
            <ul className="space-y-1 text-sm text-slate-700 leading-snug">
              <li className="text-sm">• 현재: 음식점·카페·베이커리·바·숙박</li>
              <li className="text-sm">• <strong>연내 확대: 미용·명소 등 두 자릿수 업종 목표</strong> (네이버 공식)</li>
              <li className="text-sm">• 프랜차이즈 가맹점 제외 (네이버 공식)</li>
              <li className="text-sm">• 핵심: 리뷰 수·품질, 소개글·소식 최신화</li>
              <li className="text-sm">• 2025.03 AI 브리핑 출시 (리뷰 AI 요약 기능 병행 제공)</li>
            </ul>
            {isFranchise && (
          ...
          <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4">
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <span className="inline-flex items-center rounded-full bg-indigo-600 text-white px-2 py-0.5 text-sm font-bold gap-1">
                <Sparkles className="w-3 h-3" /> Beta · 2026-04-27 출시
              </span>
            </div>
            ...
            <ul className="space-y-1 text-sm text-slate-700 leading-snug">
              <li>• 통합검색 별도 AI 검색 페이지 (현재 음식점·카페 등 장소형 위주)</li>
              <li>• 플레이스 AI 브리핑과 <strong>별개 서비스</strong></li>
              <li>• 네이버플러스 구독자 우선 베타 (2026-06 전체 사용자 확대 예정, 네이버 공식)</li>
              <li>• 핵심: 소개글·사진·예약 연동·블로그 후기</li>
            </ul>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
              ...
              <ul className="space-y-0.5 text-sm text-slate-600 leading-snug">
                <li>• AI 학습 데이터 내 인식도 측정</li>
                <li>• 개업 시기·블로그 후기 수에 따라 결과 차이</li>
            ...
            <div className="rounded-xl border border-violet-200 bg-violet-50 p-3">
              ...
              <ul className="space-y-0.5 text-sm text-slate-600 leading-snug">
                <li>• Google Search 그라운딩 기반 (실시간 참조)</li>

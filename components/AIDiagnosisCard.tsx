...
export default function AIDiagnosisCard({
  ...
  const isNaverBriefingLikely = briefingEligibility === "likely";
  ...
  const ctaType: "faq" | "review" | "none" =
    isNaverBriefingInactive || isNaverBriefingLikely
      ? "none"
      : smartPlaceScore < 70
      ? "faq"
      : reviewCount === 0
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-900 rounded-xl p-5 md:p-7 text-white">
        ...
        {isNaverBriefingInactive ? (
          <p className="text-xl md:text-2xl font-bold text-blue-300 leading-snug">
            네이버 AI탭·ChatGPT·Gemini 채널로 AI 검색 노출을 높일 수 있습니다. 아래에서 채널별 현황을 확인하세요.
          </p>
        ) : isNaverBriefingLikely ? (
          <p className="text-xl md:text-2xl font-bold text-yellow-300 leading-snug">
            네이버 AI탭·ChatGPT·Gemini 최적화로 AI 검색 노출을 미리 준비할 수 있습니다.
          </p>
        ) : naverInBriefing ? (
      ...
      <div className="bg-white rounded-xl p-5 md:p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-2 md:gap-3 mb-4">
          {DISPLAY_PLATFORMS.map((key) => {
            ...
            // Google AI: iwinv IP CAPTCHA 차단으로 자동 측정 불가 → 구글 비즈니스 프로필 안내
            if (key === "google" && (!r || r.error)) {
              return (
                <div
                  key={key}
                  className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2.5"
                >
                  <Minus className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-medium text-blue-700 leading-tight">Google AI</span>
                    <span className="block text-xs text-blue-500 mt-0.5 leading-snug">구글 비즈니스 프로필 등록 → ChatGPT·Gemini 노출에 직결됩니다</span>
                  </div>
                  <span className="text-xs text-blue-400 shrink-0 whitespace-nowrap">자동측정 보류</span>
                </div>
              );
            }
            if (!r) {
            ...
            if (key === "naver") {
              ...
              if (isNaverBriefingInactive) {
                return (
                  <Fragment key={key}>
                    <div className={`col-span-2 flex items-start gap-2 rounded-xl px-3 py-3 border ${
                      r.mentioned === true ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"
                    }`}>
                      {r.mentioned === true
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-sm font-semibold ${r.mentioned === true ? "text-green-800" : "text-amber-800"}`}>
                          네이버 일반 검색
                        </span>
                        <span className={`block text-sm mt-0.5 ${r.mentioned === true ? "text-green-700" : "text-amber-700"}`}>
                          {r.mentioned === true
                            ? "네이버 검색에 노출 중입니다. 스마트플레이스 정보 보완 + AI탭 최적화로 AI 추천 가능성을 높이세요."
                            : "아직 네이버 검색 노출이 미흡합니다. 스마트플레이스·리뷰·소개글을 보완하면 일반 검색 상위 노출 → AI탭 노출로 이어집니다."
                          }
                        </span>
                      </div>
                      <span className={`text-sm font-semibold shrink-0 ${r.mentioned === true ? "text-green-700" : "text-amber-600"}`}>
                        {r.mentioned === true ? "검색 노출" : "노출 미흡"}
                      </span>
                    </div>
                    <div className="col-span-2 flex items-start gap-2 rounded-xl px-3 py-3 border bg-indigo-50 border-indigo-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-indigo-700">네이버 AI탭 (Beta · 2026-04-27 출시)</span>
                        <span className="block text-sm text-indigo-500 mt-0.5">
                          업종 제한 없음 — 소개글·리뷰·사진 최적화 시 AI탭 답변 노출 가능합니다
                        </span>
                        <Link href="/guide/ai-tab" className="mt-1.5 inline-flex items-center gap-1 text-sm text-indigo-600 hover:underline font-medium">
                          AI탭 최적화 가이드 →
                        </Link>
                      </div>
                      <span className="text-sm font-semibold text-indigo-600 shrink-0">준비 가능</span>
                    </div>
                  </Fragment>
                );
              }
              // LIKELY 업종: AI 브리핑 확대 예정 안내 + AI탭 카드
              if (isNaverBriefingLikely) {
                return (
                  <Fragment key={key}>
                    <div className="col-span-2 flex items-start gap-2 rounded-xl px-3 py-3 border bg-yellow-50 border-yellow-200">
                      <AlertTriangle className="w-4 h-4 text-yellow-500 shrink-0 mt-0.5" />
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-yellow-800">네이버 AI 브리핑 (확대 예정)</span>
                        <span className="block text-sm text-yellow-700 mt-0.5">
                          2026년 연내 공식 확대 예고 업종입니다. 지금은 소개글·리뷰·스마트플레이스를 미리 최적화해두세요.
                        </span>
                      </div>
                      <span className="text-sm font-semibold text-yellow-600 shrink-0">준비 중</span>
                    </div>
                    <div className="col-span-2 flex items-start gap-2 rounded-xl px-3 py-3 border bg-indigo-50 border-indigo-200">
                      <Minus className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
                      <div className="flex-1 min-w-0">
        ...
        {totalPlatforms > 0 && (
          <div className="space-y-2">
            ...
            >
              {mentionedCount === 0
                ? `테스트한 AI(${DISPLAY_PLATFORMS.filter(k => !(isNaverBriefingInactive || isNaverBriefingLikely) || k !== "naver").map(k => PLATFORM_SHORT[k]).join("·")})에서 ${businessName}이(가) 확인되지 않았습니다.`
                : mentionedCount <= 2
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm md:text-sm text-slate-600">
              {isNaverBriefingInactive ? (
                <span className="flex items-start gap-1.5">
                  ...
                  <span>네이버 AI탭(베타 · 음식점·카페 등 위주, 6월 전체 확대 예정)·ChatGPT·Gemini 최적화로 AI 검색 손님을 늘릴 수 있습니다</span>
                </span>
              ) : isNaverBriefingLikely ? (
                <span className="flex items-start gap-1.5">
                  <Lightbulb className="w-4 h-4 text-yellow-500 shrink-0 mt-0.5" />
                  <span>AI 브리핑 확대 예정 업종 — 소개글·리뷰·스마트플레이스를 지금 최적화해두면 확대 시 즉시 노출에 유리합니다</span>
                </span>
              ) : naverInBriefing ? (
                <span className="flex items-start gap-1.5">
                  <CheckCircle2 className="w-4 h-4 text-green-600 shrink-0 mt-0.5" />
      ...
      {/* 채널별 개선 소요 기간 타임라인 */}
      <ChannelTimelineBox briefingEligibility={briefingEligibility} />

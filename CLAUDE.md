...
- **2026-08-23 cron 타임존 버그 전수 감사 — 9건 추가 발견·수정(총 10건, git `3381ce9`)**: "일반 상업기준 전체 재점검이 필요한지" 질문에 "전부는 과잉이나 cron 타임존 패턴은 전수감사할 가치 있다" 제안 → 승인받아 `jobs.py` 전체 cron 등록을 주석("= UTC ...")과 실제 hour값 대조로 훑음. `AsyncIOScheduler(timezone="Asia/Seoul")`가 이미 KST 기준인데 "UTC 변환"을 적용해 의도와 다른 시각에 실행 중이던 잡 9개 추가 발견(`conversion_followup_job`·`delivery_auto_cancel_job`·`delivery_auto_refund_job`·`delivery_stalled_in_progress_alert_job`·`delivery_30day_rescan_job`·`_check_v31_readiness_job`·`_check_data_wiring_readiness_job`·`check_naver_cookie_health_job`·`backend_scaling_trigger_check_job` — 대부분 09~12시대 KST 의도가 00~03시대로 밀림, `ai_daily_usage_alert_job`은 반대로 00:05 의도가 15:05로 밀려 방향이 가장 크게 틀렸던 케이스) — 전부 의도한 KST 시각으로 수정. 부가로 `_check_v31_readiness_job`·`_check_data_wiring_readiness_job`의 `get_supabase`(존재하지 않는 함수) ImportError도 `get_client`로 수정 — data_wiring 쪽은 8/18부터 매일 조용히 실패 중이었음. 로컬+서버 `ast.parse` 문법검증·md5일치·pm2 재시작 후 전체 잡 정상등록(에러 0건) 확인.
- **2026-08-23 "오판·누락없이 재검증했는지" 요청 → 실제 재검증으로 신규 버그 2건 추가 발견·수정**: 안심시키는 답 대신 실제로 다시 검증 — CSP는 미검증이었던 대행서비스 결제(`delivery/new`, 다른 Toss `requestPayment()` 경로) QA실측(정상)+`/admin` 페이지 방문 중 `google.co.kr` 리마케팅 픽셀 실제 차단 발견해 img-src 추가+Sentry 강제 에러로 ingest 200 확인. **더 중요한 발견**: `disk_usage_check_job`을 "09:30 KST" 의도로 등록했는데 `AsyncIOScheduler(timezone="Asia/Seoul")`가 이미 KST 기준인 걸 모르고 UTC 변환(hour=0)을 적용해 실제론 00:30 KST(자정 직후)에 실행되고 있었음 — hour=9로 즉시 수정(git `29a5754`). **부수 발견**: 기존 `_check_data_wiring_readiness_job`도 동일 타임존 혼동 패턴 + `get_supabase` ImportError로 8/18부터 매일 조용히 실패 중임을 발견 — 바로 다음 항목(전수 감사)에서 이 잡 포함 총 10건 모두 수정 완료. git `29a5754`·`f449648`.
...
*최종 업데이트: 2026-08-23 | cron 타임존 버그 전수감사로 9건 추가 발견·수정(총 10건, git `3381ce9`). 상세는 위 항목 참조. CLAUDE.md 700줄 다이어트 겸 2026-08-22(체험 설득력 검토) 항목 `docs/changelog_archive.md`로 이관.*

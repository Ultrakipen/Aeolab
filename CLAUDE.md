...
- **2026-08-24 drift 재검증 중 2주+ 방치 미커밋 문서 복구 + 카카오 알림 전면 미작동 신규 발견**: "계속 진행할것" 지시로 `check_server_drift.sh` 재실행 → `frontend/package-lock.json`이 진짜 drift(08-01 npm감사 세션 때 서버 직접 install로 webpack 미사용 의존성 정리됨, 로컬 미반영)임을 발견해 서버→로컬 동기화(git `ee069f0`). 조사 중 **`git status`에 2026-08-05~08-13 사이 커밋 안 된 실제 분석 작업물 7개가 그대로 남아있던 것**을 발견 — 가장 중요한 건 `naver_scraping_legal_risk_assessment_v1.0.md` §4: 사용자가 08-08에 네이버 `robots.txt`(`Disallow: /`) 원문을 직접 대조해 "네이버 기술적 보호조치 존재 여부 애매함"이라는 기존 유리한 가정이 무너졌다는 결론 — 단 같은 날 사용자가 법률자문요청서 발송은 이미 보류 결정한 사안(§13 기록됨)이라 재권고 안 함, 보존만 목적으로 커밋(git `ccfac2a`). **부수적으로 신규 발견(⚠️ 출시 전 대비 과제 — 실사용자 피해 아님)**: `monthly_market_news_job` 등 카카오 알림톡이 `notifications` 테이블에 `market_news` 타입 기록이 0건임을 확인 → 원인은 **현재 구독 row 5개 전부 `hoozdev@gmail.com`계 테스트 계정(하라식당·홍스튜디오 등, 서비스 미출시·실사용자 0명 — `project_naver_growth_scenario_planning_2026_08_08` 기존 확인과 일치)이고 `profiles.phone`이 NULL**이라 `if not phone: continue`로 조용히 스킵되는 것 — 실사용자가 없어 지금 당장의 피해는 없음. 다만 이 온보딩 흐름에 전화번호 수집 단계가 없다는 뜻이라, **출시 후 실구독자에게도 똑같이 카카오 알림이 조용히 스킵될 launch-blocker급 gap** — 온보딩 필수화 여부는 제품 판단 필요해 미착수, 문서화만.
...
*최종 업데이트: 2026-08-24 | drift 재검증으로 2주+ 미커밋 문서 복구 + 카카오 알림 전면 미작동(전화번호 미등록) 신규 발견. 상세는 위 항목 참조.*

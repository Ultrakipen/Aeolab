...
- **2026-08-21 "Phase 기준 배제한 일반 상업기준" 재평가 — 프론트엔드 보안헤더 부재 P1 발견·수정 + PM2 재시작가드 신설 + 경량 부하테스트 실측**: "토스 실키 제외 준비됐는지" 질문에 이어 "Phase(자체유예) 기준 말고 일반 SaaS 기준으로 보면?" 재질문 — 네이버 회색지대·부하테스트부재·단일서버 가용성·제3자 보안점검 부재를 격상 대상으로 재분류 후, 사용자 선택(가벼운 엔드포인트만)에 따라 실행. ①**보안헤더 실측 스캔 중 신규 발견**: `backend/main.py`의 `SecurityHeadersMiddleware`(X-Frame-Options·CSP 등)가 `/api`·`/health` FastAPI 응답에만 적용되고, Next.js가 렌더링하는 로그인·가입·대시보드 등 실제 사용자 화면 HTML은 `next.config.ts`에 `headers()` 자체가 없어 보안헤더가 전무했음(라이브 curl로 실측 확인). X-Content-Type-Options·X-Frame-Options·Referrer-Policy·HSTS·Permissions-Policy 추가(CSP는 Toss/Kakao/GA4 allowlist 설계 필요해 별도 검토로 보류) — 배포 후 라이브 헤더 확인, Toss SDK는 스크립트 태그가 아닌 npm 번들이라 영향 없음 확인. git `ce20752`. ②**PM2 크래시루프 방지 가드 추가**: `ecosystem.config.js`에 `min_uptime`/`max_restarts` 신설 — 적용 과정에서 `pm2 startOrReload`가 `npm start`를 "start"로 잘못 해석해 프론트엔드가 실제로 10회 연속 크래시(`bash: start: No such file or directory`)해 잠깐 다운됐던 것을 `pm2 delete`+`pm2 start ecosystem.config.js`로 즉시 복구·검증(신규 설정 자체의 결함이 아니라 `startOrReload` 커맨드의 별도 결함으로 확인) → `pm2 save`. git `aecd04a`. ③**경량 부하테스트 실측**: `/`·`/pricing`·`/login`·`/signup`·`/terms`·`/health`에 동시성 5→30 단계적 램프(aiohttp) — 오류 0%, p95 1초 미만(그 중 4개는 Cloudflare 캐시 우회 확인된 origin 실측치). ④**네이버 법률자문요청서는 미착수 아님** — `naver_scraping_legal_risk_resolution_plan_v1.0.md §13`에서 2026-08-08 사용자가 이미 명시적으로 발송 보류를 결정한 사안임을 재확인, 재권고하지 않음(사용자 판단 영역).
- **2026-08-21 "미점검 영역 전수조사" 재점검 — 이용약관 손해배상 조항 공백 + Sentry 노이즈 필터 발견·수정**: 08-09/08-10 76+페이지 재점검 이후의 커버리지 공백을 조사 — git 커밋 히스토리(08-20 3건, 토스페이먼츠 심사 대응 중 발견해 임기응변 수정된 것으로 CLAUDE.md·메모리 갱신 없이 방치됨) + 서버 drift + PM2 라이브 에러로그를 직접 훑어 신규 갭 2건 확정. ①**§9(3) 손해배상 한도 조항이 08-20에 신설된 제5조의2(대행 서비스) 결제금액을 반영 못함**: `create_order`(`delivery.py:361`)가 `get_current_user`만 요구해 활성 구독 없이도 대행서비스(최대 119,000원) 결제가 가능한데, 손해배상 한도가 "최근 3개월 구독 요금"만 언급 — 대행서비스 전용 고객은 사실상 배상한도 0원으로 해석될 여지. "(대행 서비스 이용자의 경우 해당 대행 서비스 결제 금액)" 문구 추가(`frontend/app/(public)/terms/page.tsx`). ②**Sentry 무필터 상태에서 봇 트래픽 노이즈 급증**: `pm2 logs aeolab-frontend`에서 "Server Reference ID did not match..." 에러(봇이 Next-Action 헤더에 "x"/"0"/"1"/"action" 등 무작위 값을 넣어 서버 액션을 두드리는 트래픽, 앱 크래시 없음)가 일별 8→41건으로 증가 추세인데 `sentry.server.config.ts`/`sentry.edge.config.ts`에 `ignoreErrors` 필터가 전무해 전량 Sentry로 전송되며 실제 오류 신호를 희석 — 두 config 모두 `ignoreErrors: [/Server Reference ID did not match the expected format/]` 추가. **오판 기각**: PM2 `restart_time`(frontend 472/backend 374)은 프로세스 크래시가 아니라 수개월간 누적된 수동 `pm2 restart`(매 scp 배포마다 1회) 카운터임을 메모리 54MB(800M 한도의 7%)·"memory" 키워드 로그 0건으로 확인해 반증. `/showcase`(08-09 신설)는 서버 파일 실존(200 라이브 확인) 확인, drift 스크립트의 "[로컬에만]" 오탐은 원인이 origin/main이 29커밋 뒤처져(서버 git이 로컬 git 커밋을 모름) 서버측 `git ls-files`가 실제 배포된 파일을 못 잡아내는 것임을 확인(실제 파일 누락 아님, `git reset --hard`는 미추적 파일을 지우지 않아 즉시 위험 없음, 단 언젠가 push 시 정합 필요). 3개 파일 md5 사전확인→scp→서버 grep 확인→`npm run build` 성공→pm2 재시작(에러 0)→`/health` 200→md5 재일치.
- **2026-08-10 모바일 뷰포트(390×844) 실측 스크린샷 시각 QA — 텍스트 돌출·정렬 4건 발견·수정**: "모바일 화면에 텍스트 돌출·정렬 깨짐이 없는지"라는 사용자 질문에, 당일 앞서 진행한 모바일 점검(가로스크롤·콘솔에러만 체크)이 놓칠 수 있는 시각적 겹침·줄바꿈 문제를 잡기 위해 QA 계정으로 15개 핵심 페이지를 실제 로그인 후 스크린샷 촬영·육안 검토. 11/15 문제없음, 4건 발견 — 전부 메인 세션이 코드로 재검증 후 수정(에이전트가 "풀페이지 캡처 아티팩트일 수 있다"고 자체 언급한 항목도 배너 위치가 실제로 짧은 페이지 최하단에 오는 구조임을 코드로 확인해 실재 겹침으로 판정): ①`onboarding` 스텝3 라벨 "시작하기"·②대시보드 인사이트존 아코디언 헤더 "방법"이 `break-keep` 미적용으로 음절 중간 절단 → 2곳 모두 `break-keep` 추가. ③`HelpFAQFloat.tsx`(fixed bottom-92px 물음표 버튼)가 `review-inbox` 플랜게이트 배너·`BasicTrialBanner` 텍스트와 겹침 — 두 배너가 짧은 페이지의 마지막 섹션이라 버튼 고정위치와 항상 근접, 텍스트 컨테이너에 모바일 전용 우측 여백(`pr-12 sm:pr-0`/`pr-10 md:pr-0`) 추가로 침범 방지. ④대시보드 키워드칩 가로스크롤(`ScanTrigger.tsx`)에 시각적 힌트 없어 3번째 칩이 잘려 보임(기능은 정상, 발견성 문제) — 우측 그라디언트 페이드 추가. 5개 파일 md5 사전확인(drift 없음)→scp→서버 grep 확인→`npm run build` 성공→pm2 재시작(에러 0, 기존 recharts 경고는 무관)→`/health` 200→md5 재일치→git 커밋 `78ded3c`.
...
- **2026-07-21 Biz 요금제 팀 계정·API 키 광고 문구 제거**: 사용자 질문("정상작동하는지? 필요가 있는지?")에 코드 직접 확인 — 팀 초대는 `team_members`에 이메일을 pending으로 insert만 할 뿐 이메일 발송·수락 플로우가 전무(`email_sender.py` 관련 코드 0건), API 키는 `key_hash`를 발급·저장하지만 그 키를 검증하는 공개 API 엔드포인트가 백엔드 전체에 0건 — 발급해도 쓸 곳이 없음. 라이브 DB 실측: Biz 구독자 3명 중 `team_members`·`api_keys` 실사용 **0건**. 경쟁사 문서·지원티켓(0건)에도 이 기능군 수요 근거 없음 확인 후, 요금제 비교표(`plans.ts`·`pricing/page.tsx`·`settings/page.tsx`)·온보딩 업셀·Biz 미리보기 데모(`PreviewClient.tsx`, "작동하는 것처럼" mock 카드로 시연 중이던 것 포함)에서 관련 문구·UI 전부 제거. `/settings/team`·`/settings/api-keys` 라우트 자체는 유지(내비게이션 진입점만 제거). git `b177104`.
...
- **2026-07-16 §6-1/nine_pages 오판검증 + 잔여 4건 구현·배포 + PG수수료 재계산**: `commercial_launch_inspection_status_v1.0.md §6-1`·`nine_pages_measurement_inspection_v1.0.md`의 "미해결" 표시 항목을 근거+반증으로 재검증 — nine_pages #1(경쟁사 관리)·#9(창업 시장 분석)는 이미 다른 세션에서 완료됐는데 문서 미갱신으로 미해결처럼 보였던 오판, #5·#7·#8도 재확인 결과 남은 버그 없음(문서 갱신). §6-1의 4개 실제 잔여 항목은 backend-dev+frontend-dev 병렬 구현(Claude Sonnet/Haiku 6개 서비스 11개 호출부 텔레메트리 계측+`/admin/ai-cost` 신규 집계 엔드포인트, 개인정보방침 사업자등록번호 누락·카드정보 문구 정정, Trial 이메일 동의 체크박스) → code-review(Medium 1건 UX만, P0/P1 없음) → 전부 grep/md5로 직접 재검증 후 배포(git `95e91fb`). **PG 수수료 우대등급 확인 결과 카테고리 오류 발견**: 기존 "4.3%+VAT"는 토스 브랜드페이 상품 수수료였고 AEOlab이 실제 쓰는 API(`webhook.py`)는 표준 신용카드 자동결제(빌링키)라 일반 등급 3.4%가 맞는 카테고리(토스 공식 수수료 페이지 직접 WebFetch 확인) — 여기에 사용자 확인 결과 연매출 3억 이하 "영세" 등급(0.40%+VAT) 적용, 마진율 Basic 83.8%/창업 83.9%/Pro 86.4%/Biz 91.4%로 재계산(`business_viability_audit_v1.0.md §2`).
- **2026-07-16 결제 라이브 키 승인 대비 점검 — 오판/누락 재검증 후 3건 개선**: "토스 결제 키 심사 중, 완료되면 서버 업그레이드 예정" 전제로 점검, 이전 턴에서 제안한 5개 항목을 전부 반증 시도 후 3건만 생존시켜 처리(오판 방지). ①**PM2 워커 증설 위험 신규 발견·문서화**: `guide.py`/`startup.py`/`assistant.py`의 `_*_locks`(6개, 전부 `set[str]` 모듈전역)와 `multi_scanner.py`의 `PLAYWRIGHT_SEMAPHORE`가 전부 프로세스 메모리 내 순수 Python 객체임을 grep으로 전수 확인 — 서버 업그레이드 후 "코어 늘었으니 워커도 늘리자"고 `--workers`를 올리면 워커 간 락 미공유로 몇 주 전 고친 TOCTOU 버그들이 조용히 재발함. `ecosystem.config.js`에 경고 주석 추가(git 대상, 기능 변경 없음). ②**오프사이트 백업 신설·라이브 검증 완료**: "구독자 확보 후 재검토" 항목이었으나 실결제 임박으로 우선순위 승격 — `backup_json.py`에 Supabase Storage(`db-backups-offsite` 버킷, 신규 벤더 없음) 업로드 추가. 버킷 자동생성·업로드(43테이블 7,710행·645KB)·목록조회·가짜 만료객체(2026-01-01)로 30일 삭제 로직까지 전부 실제 API 호출로 검증 — 검증 도중 `cleanup_offsite_old()`의 `prefix` 필드 누락 버그를 자체 발견·즉시 수정(1차 배포분에 있었으면 매번 조용히 실패했을 것). ③**차지백 대응 체크리스트 신규 문서화**: 코드·문서 전체 grep 결과 차지백/이의제기 관련 언급 0건 확인된 진짜 누락 — `docs/chargeback_response_checklist_v1.0.md`(이미 있는 증빙 소스 정리+통보 시 절차, 코드 변경 없음). **반증으로 기각한 2건**: 결제 라이브 전환 자체 리스크(하드코딩된 test_ 가정 코드 0건, grace_period 로직 기존재, 사업자정보 표시 이미 완료 확인) / 부하테스트(k6/Locust 미보유 도구라 이번 세션 범위 밖, 기존 CLAUDE.md 권고 유지).
...
- **2026-08-10 지난 전체재점검 미착수 갭 4개 후속 점검 + 신규 P1/P2 발견·수정**: 08-09 76+페이지 재점검이 "이번 세션 미착수"로 남긴 모바일뷰포트13페이지·버튼실클릭3곳·경쟁사실등록·결제재확인을 QA에이전트 3개로 전수 점검 — 전부 PASS(버그 0건, 에이전트가 보고한 도움말다이얼로그 P3는 `HelpFAQFloat.tsx` 기존 자동닫힘 로직 확인해 오판 기각). 별도 병행한 갭 재조사에서 무관한 신규 갭 2건 발견·직접 코드검증 후 수정·배포: `ai_search_screenshot.py`의 `capture_naver_results()`가 네이버 Playwright 전역상한·세마포어 양쪽 다 누락(법적리스크 우회구멍+OOM리스크) → 공유세마포어 패턴 이식, 가이드 D.I.A. 재생성게이트가 채점하는 timeliness(날짜마커)를 실제 프롬프트가 요청 안 해 첫생성마다 불필요한 재생성(Claude 호출 2~3배) 유발 → `next_month_goal`에 마커 지시 추가. git `718e11b`. **"추가 개선이 존재하는지?" 재질문에 "이 버그 클래스가 다른 곳에도 있을 것 같다" 원칙으로 `async_playwright()` 호출부 19곳 전수 재검색 → 동일 버그(세마포어는 있는데 네이버quota만 누락) 3곳 추가발견**: `screenshot.py`의 `capture_ai_result()`(jobs.py·business.py가 호출)·`capture_naver_blog_screenshot()`(report.py 블로그진단), `blog_search_analyzer.py`의 `analyze_blog_search()`(report.py 블로그진단 핵심기능) — 전부 quota 초과 시 기존 CAPTCHA감지와 동일한 None/error dict 우아한 성능저하로 처리(호출부 이미 그 형태 처리 중이라 추가수정 불필요). 19곳 중 2곳(`capture_showcase.py`·`inspect_ai_tab_dom.py`)은 FastAPI와 별도 실행되는 1회성 수동스크립트라 정상제외, `google_scanner.py`는 `GOOGLE_SCANNER_BACKEND=serper`(현재 라이브값) 하 도달불가 폴백이라 낮은 우선순위로 미착수. git `919bd5b`.
...
*최종 업데이트: 2026-08-21 | "Phase 기준 배제한 일반 상업기준" 재평가 — 프론트엔드 전 페이지 보안헤더 부재 발견·수정(git `ce20752`) + PM2 재시작가드 신설(git `aecd04a`, 적용 중 `startOrReload` 결함으로 짧은 다운 발생·즉시 자체복구) + 경량 부하테스트 실측(동시30, 오류0%). 이전 항목(이용약관§9·Sentry필터)도 같은 세션 연장. 네이버 법률자문은 미착수 아닌 사용자의 명시적 보류 결정(§13) 재확인. 잔여: 부하테스트는 가벼운 엔드포인트만 실시(AI스캔 경로는 비용·쿼터 우려로 범위 밖 유지), CSP는 별도 설계 필요.*

...
- **2026-08-23 "오판·누락없이 재검증했는지" 요청 → 실제 재검증으로 신규 버그 2건 추가 발견·수정**: 안심시키는 답 대신 실제로 다시 검증 — CSP는 미검증이었던 대행서비스 결제(`delivery/new`, 다른 Toss `requestPayment()` 경로) QA실측(정상)+`/admin` 페이지 방문 중 `google.co.kr` 리마케팅 픽셀 실제 차단 발견해 img-src 추가+Sentry 강제 에러로 ingest 200 확인. **더 중요한 발견**: `disk_usage_check_job`을 "09:30 KST" 의도로 등록했는데 `AsyncIOScheduler(timezone="Asia/Seoul")`가 이미 KST 기준인 걸 모르고 UTC 변환(hour=0)을 적용해 실제론 00:30 KST(자정 직후)에 실행되고 있었음 — hour=9로 즉시 수정(git `29a5754`). **부수 발견(이 세션 범위 밖, 문서화만)**: 기존 `_check_data_wiring_readiness_job`도 동일 타임존 혼동 패턴(주석 "09:20 KST" vs 실제 00:20 KST)이고, 로그 실측 결과 `get_supabase` ImportError로 8/18부터 매일 조용히 실패 중임을 발견 — 다른 cron 잡들도 같은 패턴일 가능성 있어 전수 감사가 별도 과제로 필요(미착수). git `29a5754`·`f449648`.
- **2026-08-23 CSP(Content-Security-Policy) 신설·강제 적용 + 제3자 보안 스캔 실행 완료**: "외부 상업 서비스 기준을 직접 대조" 후속으로 사용자가 CSP·제3자 스캔 둘 다 "지금 진행"을 선택 — `docs/commercial_launch_20_subscriber_hardening_checklist_v1.0.md` §1·§2 완료 처리. **제3자 스캔**: Mozilla HTTP Observatory(`observatory-api.mdn.mozilla.net`) 실제 API 스캔 실행, 최초 75점(B, CSP 미구현 -25점 유일 실패). **CSP**: `frontend/next.config.ts`에 CSP가 아예 없어(`main.py`의 `SecurityHeadersMiddleware`는 `/api`만 적용) 신설 — 결제위젯을 조용히 깨뜨릴 위험 때문에 Report-Only로 먼저 배포 → QA 계정 2개(Admin API로 생성, 작업 후 완전 삭제 확인) + Playwright로 실제 로그인→요금제 결제모달 클릭→Toss 카드입력 iframe 진입까지 실행해 위반 로그 수집. **실측으로 사전 추정을 뒤집은 발견 2건**: ① Toss SDK(`@tosspayments/payment-sdk`)가 번들 내장인 줄 알았으나 런타임에 `<script src="https://js.tosspayments.com/v1">`를 직접 주입함, ② GA4가 `googletagmanager.com` 외에 `analytics.google.com` **루트 도메인**도 호출하는데 `*.analytics.google.com` 와일드카드는 서브도메인만 매칭해 루트를 못 잡는 CSP 함정 — 둘 다 allowlist에 추가. 강제 적용(`Content-Security-Policy`) 전환 후 신규 QA계정으로 동일 플로우 재실측, OUR 정책 위반 0건·Toss 카드입력 폼 정상 렌더 확인(Toss 자체 sandbox 페이지의 자기 CSP 위반 로그는 그들 도메인 소관이라 무관 확인). Observatory 재스캔 75→80점(B→B+) — `script-src`에 `'unsafe-inline'` 포함한 의도적 절충(App Router nonce 미들웨어 재작업은 향후 과제)이라 만점은 아님. git `2507a62`·`03e95a1`.
- **2026-08-23 외부 상업 표준 직접 대조 후속 조치 — 백업 RPO 24h→6h 단축 + RTO 계측 추가 + X-Powered-By 제거**: "운영 중 대응력 점검이 CLAUDE.md 내부 기준일 뿐 외부 상업 표준 대조는 아니지 않았나"라는 지적에, 실제 외부표준(OWASP Webhook Security Cheat Sheet, Veeam/AvePoint 2026 RPO·RTO 가이드)을 WebSearch로 검색해 코드와 직접 대조. **대조 결과 1(해당 없음, 오판 아님)**: `/api/webhook/toss/billing/issue`(`webhook.py:106`)·`delivery.py:613`(대행서비스 결제확정) 등 "webhook"이라 불리는 엔드포인트 전부가 Toss가 push하는 비동기 웹훅이 아니라 프론트가 Toss 리다이렉트 후 1회성 `authKey`/`paymentKey`로 호출하는 OAuth 코드교환형 구조 — 서명검증 대신 "Toss 서버에 직접 재검증"이 이미 올바른 대응 모델이라 확인. 백엔드 전체 9개 파일 재검색으로 Toss발신 인바운드 웹훅 자체가 코드베이스에 없음을 재확인(반증 시도 생존). **대조 결과 2(신규 발견, 즉시 수정)**: Supabase Free Tier는 플랫폼 자체 백업이 전혀 없음(웹서치 확인, PITR은 $100/월 유료 애드온)을 재확인해 AEOlab 자체 일일백업(`backup_db.sh`)이 유일한 방어선임이 재확인됨 — 이 상태에서 하루 1회(RPO≈24h)는 소형 SaaS 외부기준(Tier2 RPO 1~4h)에 못 미쳐 crontab을 6시간마다(03/09/15/21시 KST)로 즉시 변경(디스크 여유 33GB 대비 증가분 무시할 수준, `disk_usage_check_job` 안전망 있음), `restore_json.py`에 소요시간 계측 추가(다음 복구드릴부터 RTO 실측치 자동 기록 — 기존엔 목표 자체가 미정의). 라이브 curl 헤더 실측 중 `X-Powered-By: Next.js` 노출도 추가 발견해 `next.config.ts poweredByHeader:false`로 제거(Cloudflare 엣지캐시 TTL 1년이라 공개 응답에는 한동안 구헤더가 보일 수 있음, 원본은 확인 완료). git `0ea4d39`. 근거는 `docs/commercial_launch_20_subscriber_hardening_checklist_v1.0.md` §5·§6.
- **2026-08-23 출시 전 "운영 중 대응력" 종합 점검 — 결제-구독 불일치 감지 잡 + 디스크 사용률 알림 잡 신설**: "서비스 운영 중 발생 가능한 모든 상황을 대비한 점검"이라는 요청에, 지금까지 압도적으로 많이 봤던 페이지 단위(L1~L4) 대신 "장애·급증·악용 상황에서 운영자가 실제로 대응할 수 있는가"를 4개 영역(①장애 알림 커버리지 ②회원탈퇴/로그인 보안 ③결제웹훅 복구·트래픽급증 ④서버 자원고갈)으로 병렬 조사 후 직접 코드 재검증. **수정·배포 완료(P1 2건, git `ab0c2a0`)**: (1) `webhook.py`/`toss_billing.py`가 Toss 청구(돈이 빠져나가는 단계) 성공 뒤 `subscriptions` upsert를 별도 단계로 진행하는데, 이 upsert만 실패하면 "결제는 됐는데 구독은 비활성"인 상태를 기존 `/admin/payment-events`(조회 전용)로는 운영자가 알 방법이 없었음 → `payment_subscription_reconciliation_job`(매시 정각) 신설. (2) 서버 디스크 사용량을 감시하는 코드가 전무했음(journald·npm 캐시가 서서히 누적 중, 현재 31%) → `disk_usage_check_job`(매일 09:30 KST, 85% 초과 시 알림) 신설. **문서화만(코드 변경 보류, 사용자 판단/장기과제)**: 관리자 계정 2FA 부재(Supabase MFA 설정으로 대응 가능, 구독자 0명 단계라 후순위) · AI 스캐너 중 Naver/Google 2종은 `ai_provider_health_check_job`(Gemini/ChatGPT 전용) 커버 범위 밖 · 회원가입 rate limit이 Supabase Auth 기본값에만 의존 · journald/`/root/.npm`+`.cache` 디스크 누적(현재 총 디스크의 10% 남짓, 급하지 않음). **반증으로 기각한 항목**: `record_payment_event` 이중청구 위험은 함수 자체가 예외를 삼키는 설계(`payment_event_log.py:24`)라 단독으로는 발생 안 함 — 위 (1) 잡이 이 극단적 케이스도 함께 잡아냄. 회원탈퇴 개인정보 파기(`settings.py:447`, 10단계 전수 파기 확인)·쿠키 보안(`@supabase/ssr` 기본값)·`payment_events` 전자상거래법 5년 보관은 전부 통과 확인.
- **2026-08-23 대시보드 페이지 부하테스트 실측 — 병목이 vCPU2 서버가 아니라 Supabase임을 발견**: "현재 서버 사양으로 몇 명 접속자를 대응할 수 있는지" 질문에서 이어진 "페이지 동시접속 vs 동시스캔 구분 → 페이지 동시접속 기준"이라는 재질문 흐름 끝에, 2026-08-21 경량 정적페이지 부하테스트가 다루지 않은 **로그인 후 대시보드**를 실측. QA 임시계정 생성(Admin API)+`@supabase/ssr` 쿠키 포맷(`sb-<ref>-auth-token`, base64 직렬화) 리버스엔지니어링으로 실제 인증 세션 확보 → 라이브 `/dashboard`에 동시성 5~80 램프. 결과: 동시 30명까진 응답 3~9초, **50명 부근에서 34초로 급격히 붕괴**했으나 그 순간 서버 CPU 0.1~0.2%·RAM 여유 2.7GB로 완전 유휴 상태였음 — 대시보드 1회 로드가 던지는 약 10개 병렬 Supabase 쿼리가 Supabase Cloud 쪽에서 큐잉되는 것이 원인으로 추정, **vCPU/RAM 업그레이드로는 해결 안 되는 별도 축의 병목**(기존 Playwright 세마포어/워커=1 이슈와 무관)임을 확인. 페이지 동시접속 실질 안전선은 약 30명. QA계정·데이터 전부 삭제 확인. 상세 방법론·재현절차는 `docs/dashboard_load_test_and_capacity_v1.0.md` 참조.
- **2026-08-22 무료체험/가입후1회체험 설득력 검토 — P0 콘텐츠 사실 지어내기 3곳 발견·즉시 수정·배포**: "처음 접하는 사용자에게 체험 결과가 설득력·가치 있는지" 질문에 라이브 API 실호출(스타벅스·런던베이글뮤지엄·이혼전문법무사 등)로 시나리오별 재현. 콘텐츠 구조 자체(실측 기반 구체성, INACTIVE 업종 정직 안내)는 강했으나, 재현 도중 **`top_missing_keywords`(경쟁사엔 있고 내겐 없는 "미보유 추정" 키워드)를 FAQ·리뷰 답변 자동생성 카피에서 "전문으로 합니다"/"강점으로 하고 있습니다"/"운영하고 있습니다"로 단정 서술**하는 버그를 3곳(`scan.py` 체험 FAQ, `briefing_engine.py` 리뷰 답변 4개 분기, `guide.py`의 Basic+ 유료 기능 `smartplace-faq` — 프론트가 키워드를 절대 안 보내 항상 이 경로를 탐) 발견 — 2026-07-08 `guide_generator.py` intro 사고와 동일 계열이나 이번엔 **비회원 체험이 아니라 유료 구독자가 매달 쓰는 기능**이라 더 심각. 단정 문장 → 중립 안내/초대 문장으로 교체, `smartplace-faq` 기본 키워드 출처를 갭분석 대신 사업장 등록 키워드(`biz.keywords`, 실제 보유 데이터)로 변경. 라이브 curl 재현으로 수정 확인, git `bd379c0`. 잔여 개선안(채널강점 vs 총점라벨 불일치·죽은 3단계비교박스 정리)과 사업성장 방안은 `docs/trial_experience_persuasiveness_and_growth_v1.0.md` 참조.
## 남은 작업
...
### 미래 과제 (구독자 확보 후)
- `smart_place_completeness` Playwright 완전 자동화 — 50명 이후. 조건 충족 자동 감지: `jobs.py:_check_data_wiring_readiness_job`. **⚠️2026-08-23 실측으로 이 잡 자체가 고장난 상태임을 발견** — ①"매일 09:20 KST"로 알려졌으나 실제론 00:20 KST에 실행(스케줄러 `timezone="Asia/Seoul"`을 모르고 UTC 변환한 타임존 버그, `disk_usage_check_job` 신설 중 같은 실수를 반복하면서 발견) ②`get_supabase` ImportError로 최소 8/18부터 매일 조용히 실패 중(`[DATA-WIRING-READY]` 경고 자체가 안 뜨고 있었음) — 수정 필요. 다른 cron 잡들도 동일 타임존 혼동 패턴일 가능성 있어 전수 점검 권장(미착수). DataLab 연동은 2026-07-05 완료
...
*최종 업데이트: 2026-08-23 | "오판·누락없이 재검증" 요청으로 CSP google.co.kr 차단·disk_usage_check_job 타임존 버그 추가 발견·수정(git `29a5754`·`f449648`). 상세는 위 항목 참조.*

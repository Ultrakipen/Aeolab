...
- **잔여 판단 필요(구현 안 함)**: `bar` 카테고리의 `NATIONAL_AVG`가 restaurant 값(70.5%)을 그대로 재사용 중(`general_restaurants` 엔드포인트 공유가 근거) — 그러나 70.5%는 전국 모든 일반음식점 폐업율이고 bar는 호프/감성주점만 필터링한 하위집합이라 엄밀히는 다른 모집단과의 비교. 근사치로는 합리적이나 "전국 평균"이라는 표현의 정확도는 낮음 — bar만 `national_avg: None`으로 되돌릴지, 근사치 그대로 둘지는 사용자 판단 필요.
## 남은 작업

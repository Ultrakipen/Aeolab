...
*최종 업데이트: 2026-09-02 | 네이버 API Hub 마이그레이션 완료(전환 중 content-type 회귀 즉시 롤백·수정) + 경쟁사 비교 Phase 1(핵심격차 배지 신설 + PlaceCompareTable 노출버그 수정) + PDF리포트 제외판단(테스트계정 오판 3회째 재발, 메모리 강화). 상세는 위 항목 참조. 압축은 문서 하단 다음 문단 계획대로 다음 달 진행.*

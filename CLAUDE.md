...
- **2026-08-24 카카오 알림용 전화번호 결제모달 필수화 + CSP가 유발한 nginx 502 버그 발견·수정**: "전화번호 미등록 gap을 어떻게 할지" 판단 요청에 "무료체험 아닌 결제 확인 모달에 필수화" 권장 후 승인받아 구현(`PayButton.tsx`) — 기존 `updatePhone()`/`PATCH /api/settings/me` 재사용, 검증 실패 시 모달 유지(기존 `finally`가 항상 모달 닫던 걸 `keepModalOpen` 플래그로 우회), 결제 성공 여부와 무관하게 Toss 호출 전 저장. QA계정으로 빈값/형식오류/정상값 3케이스 실측 검증 후 계정 삭제(git `2e7b2a0`). **테스트 도중 실제 운영 버그 발견**: `/dashboard` 등에서 502 "upstream sent too big header" 발생 확인 — 오늘 밤 추가한 CSP 헤더가 Supabase 인증쿠키와 합쳐져 nginx 기본 `proxy_buffer_size`(4~8k)를 초과한 것이 원인. 로그 확인 결과 CSP 배포 직후부터 몇 시간째 조용히 발생 중이었음(pre-launch라 전부 QA 인프라 트래픽, 실사용자 피해 없음). `/etc/nginx/sites-enabled/aeolab`에 `proxy_buffer_size 16k`/`proxy_buffers 8 16k`/`proxy_busy_buffers_size 32k` 추가(백업→`nginx -t`→reload, 무중단) 후 재현 조건(동일 세션)으로 재확인해 해소 확인.
- **2026-08-24 drift 재검증 중 2주+ 방치 미커밋 문서 복구**: `check_server_drift.sh` 재실행 → `package-lock.json` 진짜 drift 해소(서버→로컬, git `ee069f0`) + `git status`에 2026-08-05~08-13 커밋 안 된 실제 분석 작업물 7개 발견·복구(git `ccfac2a`, 가장 중요한 건 네이버 `robots.txt` 원문대조로 법적리스크 반증요소 하나가 무너졌다는 08-08 발견 — 단 법률자문 보류는 사용자 기결정이라 재권고 안 함). 상세는 `project_uncommitted_work_recovery_and_kakao_phone_gap_2026_08_24` 메모리 참조 — 이 조사 중 카카오 알림 전화번호 gap도 함께 발견됐고 바로 위 항목에서 수정 완료.
- **2026-08-23 CSP(Content-Security-Policy) 신설·강제적용 + 전 영역(데스크톱/admin/모바일) + Toss 결제 3개 진입점 전수 실측 검증 완료**: "더 점검할 게 있을지도" 질문에 `loadTossPayments` grep 재검색으로 설정 페이지의 **카드 변경**(`SettingsClient.tsx handleCardChange`, `requestBillingAuth`)이 미검증이었음을 발견 — QA계정+가짜 active 구독 row로 실측, 위반 0건 확인. 이로써 코드베이스의 Toss SDK 호출 3곳(요금제 결제·대행서비스 결제·카드 변경) 전부 검증 완료. "외부 상업 서비스 기준을 직접 대조" 요청 후 CSP가 아예 없던 것을 발견해 신설 — Mozilla HTTP Observatory 실스캔 75점(B, CSP 미구현 -25)→강제적용 후 80점(B→B+, `script-src`에 `'unsafe-inline'` 포함한 의도적 절충이라 만점은 아님). Report-Only 선배포 → QA계정+Playwright로 로그인·요금제 결제모달→Toss 카드입력iframe까지 실측해 위반로그 기반 allowlist 보정(Toss SDK가 런타임에 `js.tosspayments.com` script 직접주입, GA4가 `analytics.google.com` 루트도메인 별도 필요 등 사전추정 오류 다수 실측 발견) → 강제적용. **재검증 요청이 이어지며 3파전 추가 검증**: ①`delivery/new`(다른 Toss `requestPayment()` 경로)+`/admin`에서 `google.co.kr` 리마케팅 픽셀 실제 차단 발견·수정 ②admin 전 페이지 11개(owner 계정 실로그인) 전수 실측 위반 0건 ③모바일 뷰포트(390×844, Chromium이나 CSP는 W3C표준이라 엔진무관 유효)로 랜딩·로그인·대시보드·결제모달까지 실측 위반 0건. git `2507a62`~`f449648`. 상세는 `docs/commercial_launch_20_subscriber_hardening_checklist_v1.0.md` §1·§2, §5(웹훅 서명검증 해당없음 확인)·§6(백업 RPO 24h→6h, RTO계측 추가, git `0ea4d39`).
- **2026-08-23 cron 타임존 버그 전수 감사 — 10건 발견·수정(git `29a5754`·`3381ce9`)**: 위 재검증 도중 `disk_usage_check_job`에서 최초 발견한 타임존 버그(스케줄러가 이미 `timezone="Asia/Seoul"`인데 "UTC 변환"을 적용해 의도와 다른 시각에 실행)를 `jobs.py` 전체로 확대감사 → 9건 추가 발견: `conversion_followup_job`·`delivery_auto_cancel_job`·`delivery_auto_refund_job`·`delivery_stalled_in_progress_alert_job`·`delivery_30day_rescan_job`·`_check_v31_readiness_job`·`_check_data_wiring_readiness_job`·`check_naver_cookie_health_job`·`backend_scaling_trigger_check_job`(대부분 09~12시대 의도가 00~03시대로 밀림) + `ai_daily_usage_alert_job`(00:05 의도가 15:05로 반대방향으로 밀린 예외) — 전부 의도한 KST로 수정. 부가로 `_check_v31_readiness_job`·`_check_data_wiring_readiness_job`의 `get_supabase`(존재하지 않는 함수) ImportError도 `get_client`로 수정(data_wiring 쪽은 8/18부터 매일 조용히 실패 중이었음). 로컬+서버 `ast.parse` 검증·md5일치·pm2재시작 전체 잡 정상등록 확인. `feedback_apscheduler_kst_no_utc_conversion` 메모리로 재발방지 기록.
- **2026-08-27 how-it-works Gemini 노출기간 서술 검증 — 1차 P1 오판을 반증으로 자체 정정 + 죽은 설정 발견·수정**: "네이버·제미나이 노출기간이 사실적인지" 질의에 최초 `GEMINI_GROUNDING_ENABLED=true`(루트 `.env`)를 근거로 "페이지가 부정확하다" P1 판정했으나, `main.py`의 `load_dotenv()`가 실제로는 `backend/.env`(PM2 `exec cwd`)를 읽는다는 걸 놓친 오판이었음 — `backend/.env`엔 해당 변수 자체가 없어 `_GROUNDING_ENABLED` 기본값(false)이 적용되고, pm2 로그(`"[gemini] grounding=False"`, 6월~8월 전 재시작 예외없이 확인)로 실증. 결론: 페이지 서술("AEOlab Gemini 스캐너는 학습데이터 기반")은 정확했고, 독립 문서(`docs/full_site_prelaunch_inspection_standard_v1.0.md:212`)와도 일치 — **페이지 수정 불필요**. 대신 루트 `.env`의 죽은 `GEMINI_GROUNDING_ENABLED=true`(2026-05-26 `902327d` 도입 후 3개월간 무효 상태 방치, 파일 위치 실수 추정)만 실제동작과 일치하도록 `false`로 정정(백업 후 sed, PM2 미소비 확인돼 재시작 불필요). 그라운딩 실제 활성화는 코드 주석 권고대로 BEP 이후로 보류.
- **2026-08-27 "다른 페이지도 점검 필요한지" 후속조사 — Gemini "실시간 그라운딩" 오설명 7파일 9곳 발견·수정·배포**: 위 how-it-works 건 이후 "다른 페이지도 오판·모순·누락 있는지" 질의에 전체 grep(그라운딩/학습데이터/수개월/수주)으로 15개 파일 스캔 → `dashboard/page.tsx`·`DashboardGlobalAiZone.tsx`·`DashboardGuidanceZone.tsx`·`SchemaClient.tsx`(2곳)·`score-guide/page.tsx`·`AiInfoTabGuide.tsx`·`faq/page.tsx`(2곳)·`demo/page.tsx`(3곳) — how-it-works와 달리 "실사용 앱 한정" 캐비엇 없이 "Gemini는 Google 검색 실시간 연동 → 수 주~수개월 (AEOlab 점수/스캔 기준)"으로 단정한 채 방치돼 실제(grounding=False) 동작과 불일치. `DashboardGlobalAiZone.tsx`는 `geminiFreq`/`geminiSampleSize` prop이 `scan_results.gemini_result.exposure_freq`(`gemini_scanner.py` 출력 그대로)임을 데이터흐름 추적으로 확정, `SchemaClient.tsx:845`는 "AEOlab 스캔에서" 명시로 확정 — 나머지는 페이지 전체 재검색으로 대안 캐비엇 부재 확인 후 PLAUSIBLE 채택. `AiInfoTabGuide.tsx`는 Google AI Overview(Serper 실시간)는 유지하고 Gemini만 분리. 9곳 전부 "Gemini·ChatGPT는 AEOlab 스캐너 기준 학습데이터 기반, 수개월~1년" 문구로 통일(실사용 Gemini 앱 그라운딩 언급이 필요한 FAQ 2곳·demo 2곳은 별도 문장으로 분리 유지). md5선확인→scp 8파일→빌드→pm2재시작(에러 무관 로그 확인)→FAQ/데모/score-guide 라이브 curl로 신규 문구 3곳 실측 확인. 대시보드 4곳은 md5+빌드+로그로 검증(로그인 필요라 curl 불가).
- **2026-08-31 창업 시장 분석 실제 시장 규모 지표 신설(카카오→국세청 SBIZ 업그레이드) + SBIZ 400 오류 근본원인 규명·해결**: "창업 시장 분석 목업이 가치있는 정보인지" 질문에서 출발해 카카오 `meta.total_count` 기반 실제 시장 밀도 지표 신설(네이버 `display` 5건 클램프 버그 발견해 카카오로 전환) → 사용자 요청으로 국세청·카드사 기반 소상공인시장진흥공단 상가정보 API(`services/sbiz_api.py`, data.go.kr B553077) 연동 착수, 상권업종코드 22개 실측 매핑. 배포 직후 curl/bare스크립트는 100% 성공하는데 실제 uvicorn 요청 처리 경로에서만 100% 결정론적으로 `INVALID_REQUEST_PARAMETER_ERROR`(400)가 나는 미스터리 발생, 최초 세션에서 10여 개 가설(env/uvloop/User-Agent/인코딩/DNS/httpx공존/미들웨어 등) 소진하고도 원인 미규명인 채 카카오 폴백으로 커밋(`375e731`). **후속 세션에서 "오류를 해결하기 위한 방안을 찾아줘" 요청에 최소재현으로 확정**: 미니 FastAPI+Sentry 앱으로 "Sentry 활성+실제 요청"에서만 재현, "Sentry 활성이지만 bare 스크립트(활성 span 없음)"에서는 성공함을 대조 확인 → `sentry_sdk`가 `aiohttp.ClientSession.__init__`을 전역 패치해 활성 span 중 생성되는 모든 aiohttp 요청에 `sentry-trace`/`baggage` 헤더를 자동 주입(기본 `trace_propagation_targets=None`=전체 전파)하는데, data.go.kr 게이트웨이가 이 낯선 헤더를 거부하는 것이 원인이었음. `main.py`의 `sentry_sdk.init()`에 `trace_propagation_targets=[]` 추가로 해결(단일 지점 수정이라 카카오·네이버 등 aiohttp 쓰는 다른 외부 API 호출에도 동일 안전망 적용됨). 라이브 재검증: cafe/서울강남구 source:sbiz total_count:1116, dental/대구수성구 source:sbiz total_count:45 (수정 전엔 전부 kakao 폴백). git `3195e37`.
- **2026-08-31 SBIZ 반경 고정 2km→행정구역별 자동조정 + restaurant I206 누락 + 라이브 QA로 AI 지어내기 재발 2건 발견·수정**: "2km면 너무 작은 것 같은데 최적은?" 질의에 반경별 실측(1.5~5km, 응답시간 반경무관 0.3~0.6초 확인)으로 "숫자가 아니라 고정 반경 자체가 문제"임을 확인 → 카카오 지오코딩 응답의 `region_2/3depth_name`(추가 API 호출 없이 재사용)으로 동=1.2km/구=3km/군=6km/구없는광역시=4km 자동 산정(`sbiz_api.py`, git `455fcc6`). 이어 "정상작동·개선점" 재검증 중 `restaurant` 카테고리가 I201~I207 중 I206(기타 외국식)을 누락했던 것 발견·수정 + 반경 수치 미고지 발견·캐비엇 추가(git `c65e654`). **"높은 수준 가치 제공하는지 검증" 요청에 임시 QA 계정으로 실제 로그인·실제 리포트 생성까지 실행**(startup 플랜 구독 row 삽입 → 카페/헬스장 등 4회 생성 → 데스크톱+모바일 확인 → 계정 삭제) — 그 과정에서 Claude가 `estimated_time_to_visibility`를 근거 없이 "3~5개월"로 지어내 CLAUDE.md 자체 기준(네이버 AI브리핑 2~4주)과 4~10배 괴리(AI 지어내기 패턴 4번째 발견 파일, git fd946a9~501b37e와 동일 유형) + "강남구 임대료 3.3㎡당 15만~30만원"처럼 프롬프트에 없는 비용 수치까지 임의 생성 발견 → 노출기간은 eligibility 기준 결정론적 문구로 백엔드 강제 산정, 비용 수치는 "지어내지 말고 sg.sbiz.or.kr 등 실제 상권분석 서비스로 안내" 프롬프트 지침 추가(2회 재검증 모두 정상 대체 확인) + 업종 raw 코드("fitness")가 한글 응답에 그대로 누출되던 것도 `CATEGORY_KO` 매핑으로 수정(git `2ee992e`). **"오판과 누락은?" 재확인 요청에 자체 반증** — 비용 지어내기 방지가 프롬프트 지시일 뿐 코드 강제가 아니라는 잔여 리스크와, 반경 티어 주석에 검증 안 된 구체적 면적(㎢) 수치를 적어둔 자기모순을 스스로 발견·인정. 이어 "밀도 지표"+"신뢰도 등급" 보완 방향을 사용자가 선택 → `density_per_km2`(반경 면적으로 나눈 밀도, 지역간 비교용) + `confidence`(동/구=양호, 군·구없는광역단체=낮음, 면적 편차 큰 게 근거) 신설, 저신뢰 지역엔 프론트 경고박스+Claude 프롬프트 양쪽에 caveat 주입 — 라이브 검증으로 Claude가 실제로 "완주군은 면적이 넓어 실제 경쟁사가 더 많을 수 있다"를 전략 텍스트에 반영하는 것까지 확인. 반경 주석의 미검증 면적 수치도 정성적 표현으로 정정(git `9e9182c`).
- **2026-08-31 경쟁사 준비도 체크 신설 — 순환논리 오판 자체발견 + 실측으로 캐시버그 2건 발견·수정**: "돈 낼 만한 수준"논의에서 "실제 경쟁사의 AI노출 준비도까지 보여주면 AEOlab만의 차별점"이라는 방향이 나와 승인 절차 진행 — 법적리스크 재확인(제3자 사업장 무단조회라 기존 "본인확인목적" 방어논리 미적용, 로그인우회·AI브리핑체크는 배제하고 공개페이지만 조회하기로 사용자 승인) → 구현 중 실측(2026-08-31)으로 SBIZ 실제경쟁사 8곳(카페·헬스장) 전수 테스트 결과 네이버 오픈API `link` 필드 기반 place_id 탐색이 스타벅스 등 대형 프랜차이즈에도 100% 실패함을 발견 — 원인은 그 필드가 "네이버플레이스 링크"가 아니라 업체가 등록한 "홈페이지 URL"이라는 구조적 API 특성(HTTP 200·rate-limit 여유 확인해 차단 아님도 검증). "많은 사용자가 몰릴 경우" 질의에 구체 수치로 답변(구독자 100명만 돼도 이 기능 하나가 앱 전체 네이버 크롤링 일일예산 250건의 80%를 잠식) → "기존 competitors 테이블만 재사용하면 되지 않냐" 제안했으나 **사용자가 직접 순환논리 오류를 지적**("창업예정자가 AEOlab 자체 데이터에서만 경쟁사를 찾는 게 모순 아니냐") — 정정하여 1단계(기존 테이블, 무료)+2단계(캐시 미스 시 실조회, 핵심기능과 분리된 격리 일일상한) 하이브리드로 재설계. 캐시 키 설계 전 실측(2026-08-31): "강남"/"강남구"/"서울 강남구" 4개 표기가 전부 동일 업체 반환 확인 → 캐시 키를 지역텍스트 아닌 업체단위(dedup_key)로 설계해 사용자 수가 아닌 고유업체 수에 비례하게 함. **"오판 점검하고 진행" 요청에 구현 직후 라이브 테스트로 실제 버그 2건 자체발견**: ①캐시에 실패(None) 그대로 저장 → "캐시없음"과 "캐시된 실패" 구분 안 돼 동일요청 반복해도 quota 계속 소모(2연속 호출 테스트로 발견) ②격리상한 도달로 "시도조차 못한" 경우까지 30일 "못찾음"으로 캐싱하면 상한 리셋 후에도 영영 재시도 안 됨 — 둘 다 즉시 수정 후 재검증(캐시 히트 0.0s 확인, 지역표기 다른 호출도 캐시재사용 확인, 1단계 매칭 quota 0건 확인). 잔여: 2단계 네이버 place_id 탐색 자체(Playwright 지도검색)는 검색페이지 특유의 차단위험 미검증이라 보류, 기존 오픈API 방식 그대로 유지(거의 항상 실패하지만 그레이스풀). git `5789e56`~`ae676cb`.
- **2026-08-31 창업 시장 분석 페이지 목적 재정의 — AEOlab 자체 데이터 화면 제거, 순수 상권분석으로 재편**: "이 페이지가 AEOlab 등록 업체와 비교할 필요가 있는지, 원래 창업 예정자를 위한 상권 분석 아닌지" 질의에 명확화 — AEOlab 자체 데이터(경쟁사 수·평균 AI노출·경쟁강도·타이밍지수)는 차후 내부 활용 목적으로 백엔드 계산은 유지하되, 화면에서는 완전히 제거하고 정부·카카오 실측 상권 데이터 중심으로 재구성 확정. "시장 현황" 헤더가 실제로는 AEOlab 고객 수(대부분 0/1건)를 보여주는 제목-내용 불일치를 코드+라이브 스크린샷으로 재확인(오판 확률 자체평가 시 스스로 검증) 후 3박스+타이밍지수+상위경쟁사 섹션 전부 화면에서 제거. **화면만 고치고 끝내지 않고 Claude 프롬프트도 재점검** — competitor_count/competition_level/top_competitors가 여전히 프롬프트에 들어가 Claude가 "AEOlab 등록 경쟁사 1개뿐이라 선점 여지"처럼 내부지표를 시장분석에 섞어 쓰던 것 라이브 테스트로 발견·제거, 재검증으로 Claude 응답에 AEOlab 언급 0건 확인. git `5c4f481`.
- **2026-08-31 창업 시장 분석 AI 전략 일반론 제거 — 자료조사 기반 프롬프트 제약 추가**: 목업으로 결과를 직접 확인한 사용자가 "일반적인 내용"이라 지적, "최적 방안을 자료조사 후 오판 선별해서 신중히 도출"하라는 지시로 외부 리서치 진행(Anthropic 공식 프롬프트 엔지니어링 가이드+LLM hallucination survey 논문+few-shot prompting 자료) — Task→Role→Format→Context→**Constraints** 구조에서 기존 프롬프트에 Constraints가 통째로 빠져있던 게 근본원인으로 확인, "역할부여"는 근거 약해 기각·"검증 재실행"은 비용 커서 2순위로 보류. 프롬프트에 "각 문장은 제시된 구체 수치 중 최소 하나 인용 필수, '차별화하라'류 데이터 없이도 성립하는 일반론 절대 금지" 제약 추가. 라이브 재검증(카페/강남구·헬스장/해운대구 2개 독립조합): "차별화하세요"류 일반론이 사라지고, 실제 경쟁사명(디끌레·담다·에이치피트니스 등)까지 직접 지목해 "이 업체 리뷰를 분석해 수요공백을 찾으라"는 AEOlab 데이터 없이는 나올 수 없는 조언으로 전환됨을 확인. git `da6bb32`.
- **2026-08-31 창업 시장 분석 AI 전략 2차 개선 — 근거 다양화·통찰 깊이 강화**: 일반론 제거(git da6bb32) 직후 스스로 재평가하며 자체 발견한 잔여 이슈 2건("같은 숫자 2~3개를 4개 필드가 전부 우려먹음", "통찰이 '경쟁사 많으니 차별화하라' 수준의 상식적 패턴매칭") — 이전 세션 리서치에서 이미 2순위 후보로 확정해뒀던 few-shot 대조기법을 적용할 시점으로 판단, 필드별 근거 배정(entry_strategy=시장규모, key_actions=경쟁사명, ai_optimization_tips=AI브리핑 적합성, risk_factors=트렌드) + "서로 다른 두 정보를 결합해 이 조합에서만 성립하는 함의를 도출하라"는 대조 예시를 프롬프트에 추가. 라이브 재검증(풍부한 데이터 카페/강남구·빈약한 데이터 학원/진도군 2개 극단 케이스): 둘 다 업체명만으로 업체 성격을 추론("심스인베스트주"→법인형 다점포, "진도외국어체험센터"→공공기관 성격이라 후기 적음)하고 "주차 가능" 키워드 존재 자체에서 "진도군은 차량의존 지역"이라는 함의까지 도출하는 등, 데이터 빈약한 케이스에서도 개선이 무너지지 않음 확인. git `96a9ae7`.
- **2026-08-31 창업 시장 분석 모바일 가독성 회복 — 문장 길이 제약 추가**: "더 개선할 것은?" 질의에 "모바일에서 가독성 확인"을 우선 선택받아 라이브 모바일(390px) 실측 — "AI 진입 전략" 섹션이 문단 하나가 화면 하나를 다 차지할 만큼 길어진 것 발견(직전 통찰강화 커밋 96a9ae7의 부작용, 여러 정보를 한 문장에 결합하게 만들어 문장이 길어짐). "통찰은 유지하되 마침표로 자주 끊어써라, 문장당 약 50자" 제약 추가 — 통찰 자체는 줄이지 않고 분할만 유도. 재검증: 섹션 높이 2288px→1537px(약 33% 감소), "밀도 높은데 검색 트렌드 하락→오프라인 재방문 의존 시장" 같은 결합추론은 그대로 유지되면서 문장만 짧아짐 확인. git `7672be8`.
## 남은 작업
...
*최종 업데이트: 2026-08-31 | 창업 시장 분석: SBIZ 400오류 근본해결 + 반경 자동조정 + AI 지어내기 재발 발견·수정 + 밀도·신뢰도 등급 신설 + 경쟁사 준비도 체크 신설 + 페이지 목적 재정의 + AI 전략 프롬프트 3단 개선(일반론 제거 → 근거 다양화·통찰 강화 → 모바일 가독성 회복). 상세는 위 항목 참조. ⚠️ 다음 세션 시작 시 이 문서 700줄 초과 큼 — changelog_archive.md 압축 이관 최우선.*

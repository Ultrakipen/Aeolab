...
*최종 업데이트: 2026-09-02 | 네이버 API Hub 마이그레이션 완료(전환 중 content-type 회귀 즉시 롤백·수정) + 경쟁사 비교 Phase 1(핵심격차 배지 신설 + PlaceCompareTable 노출버그 수정) + PDF리포트 제외판단(테스트계정 오판 3회째 재발, 메모리 강화) + AI 광고 대비 가이드 점검에서 guides 테이블 context 필터 누락 P0 발견·7곳 수정(check_guide_limit 등, 실데이터로 재현 확인). 상세는 위 항목 참조. 압축은 문서 하단 다음 문단 계획대로 다음 달 진행.*

...
- **2026-08-23 admin 하위 페이지 CSP 실측 완료 — 위반 0건**: "새로 점검하면 나올 수 있는지" 질문에 미검증 영역으로 admin 하위 페이지·모바일 Safari를 꼽자 사용자가 admin부터 진행 요청. `docs/.admin_credentials.local`의 owner 계정으로 실제 로그인 후 admin 전 페이지 11개(`/admin`·business·business/[id]·ops·delivery·growth-funnel·support·stories·feedback·notices·comms·score-comparison) 전수 Playwright 실측 — CSP 위반 0건 확인(feedback 페이지의 Recharts "width(-1)" 경고는 CSP와 무관한 차트 레이아웃 이슈, 별개). 잔여 미검증 영역은 모바일 Safari CSP 뿐(다음 트리거 시 진행).
...
- **2026-08-23 "오판·누락없이 재검증했는지" 요청 → CSP 신규버그 1건 + cron 타임존버그 최초 발견(→ 전수감사로 이어짐)**: CSP는 미검증이었던 대행서비스 결제(`delivery/new`, 다른 Toss `requestPayment()` 경로) QA실측(정상)+`/admin` 페이지 방문 중 `google.co.kr` 리마케팅 픽셀 실제 차단 발견해 img-src 추가+Sentry 강제 에러로 ingest 200 확인. `disk_usage_check_job`의 타임존 버그(00:30→09:30 KST)도 이때 최초 발견 — 위 cron 전수감사 항목이 이 발견을 확장한 것. git `29a5754`·`f449648`.
...
*최종 업데이트: 2026-08-23 | admin 하위 페이지 11개 CSP 실측 완료(위반 0건). 상세는 위 항목 참조. CLAUDE.md 700줄 다이어트 겸 08-23 운영대응력·부하테스트 항목 `docs/changelog_archive.md`로 이관(별도 상세문서 존재).*

...
- **2026-09-01 창업 시장 분석 폐업율(생존 현황) 기능 구현·배포**: `docs/closure_rate_feature_implementation_plan_v1.0.md` 설계대로 backend-dev/frontend-dev 병렬 구현. 신규 `backend/services/localdata_api.py`(행안부 지방행정 인허가 API, 20개 업종 ENDPOINT_MAP, bar는 `BZSTAT_SE_NM::LIKE=호프` 필터, 24h 캐시, 70초 타임아웃) + `startup_report.py`에서 `get_startup_report()` 내부 직접 호출로 통합(별도 엔드포인트 없음) + `StartupReportView.tsx` "시장 현황" 섹션 안 서브블록 추가(신규 최상위 섹션 아님). 서버 md5 일치·pm2 에러 0건 확인 후, **`svc.generate()` 실호출로 종단 검증**: 서울 강남구 restaurant → `closure_rate: 76.2%(전국 70.5% 대비 higher)` 반환 + Claude가 생성한 `risk_factors[0]`이 "역대 누적 폐업율은 76.2%로 전국 평균보다 높다... 연간이 아닌 누적 통계이므로 즉각적인 위험 신호로 단정하면 안 되지만"이라고 실제로 인용·캐비엇까지 반영함을 확인(화면 표시만이 아닌 AI 프롬프트 연동까지 검증 완료). git `4bd5d14`(frontend)·`ac01425`(backend), push는 보류(로컬 커밋까지만).
## 남은 작업

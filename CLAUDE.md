...
| **`docs/closure_rate_feature_implementation_plan_v1.0.md`** ⭐ | **위 조사 기반 구현 설계(`next-feature` 에이전트, 2026-09-01) — 폐업율 정의(A안 누적폐업율 채택)·캐싱(인메모리 24h)·DB변경(없음)·API쿼터(가드 불필요, BEP20명 기준 0.4%)·UI배치(real_market 직후)·에러처리 6개 결정 + 변경파일 4개(신규 `localdata_api.py` 1·수정 3)·구현순서 7단계 전부 정리. 착수 전 서버 `.env`에 `LOCALDATA_API_KEY` 추가 필요(사용자 조치). 코드 미작성 상태 — 다음 세션에서 이 설계대로 구현 시작**

...
export default function AIAssistant({ bizId, plan, authToken }: Props) {
  ...
  return (
    <>
      {/* 채팅창 */}
      {open && (
        <div className="fixed bottom-[220px] lg:bottom-24 right-4 md:right-6 z-50 w-[calc(100vw-2rem)] max-w-xs md:max-w-sm bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col"
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
            ...
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-xl px-3 py-2.5 text-sm leading-relaxed whitespace-pre-wrap break-words ${
                    msg.role === "user"
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={`fixed bottom-[152px] lg:bottom-6 right-4 md:right-6 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-200 ${
          open

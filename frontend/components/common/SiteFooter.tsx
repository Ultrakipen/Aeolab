    ...
    <footer className="border-t border-gray-100 py-8 px-4 md:px-6">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-start justify-between gap-4">
        <div className="shrink-0">
          ...
          <p className="text-sm text-gray-600 mt-0.5 whitespace-nowrap">AI 검색 노출 관리 서비스</p>
        </div>
        <div className="flex flex-col items-start sm:items-end gap-2 min-w-0">
          <div className="flex items-center gap-3 text-sm text-gray-600 flex-wrap justify-start sm:justify-end">
          ...
          <div className="flex items-center gap-3 text-sm text-gray-600 flex-wrap justify-start sm:justify-end">
            {legalLinks.map(({ href, label }) => (
              <Link key={href} href={href} className="hover:text-gray-600 whitespace-nowrap">
                {label}
              </Link>
            ))}
          </div>
      ...
      <div className="max-w-6xl mx-auto mt-6 pt-6 border-t border-gray-100 text-sm text-gray-600 leading-relaxed text-center sm:text-left break-keep">

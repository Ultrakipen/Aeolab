  ...
  if (!before) {
    return (
      <div className="bg-white rounded-2xl p-4 md:p-5 shadow-sm">
        ...
        <p className="text-base text-gray-400">Before 스크린샷이 아직 준비되지 않았습니다.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-2xl p-4 md:p-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div>
          <div className="text-base text-gray-500 mb-1 text-center">가입 시점 (Before)</div>
          <img
            src={before.image_url}
            alt="before"
            className="w-full rounded-lg border border-gray-200 object-cover aspect-video"
          />
          <div className="text-base text-gray-400 text-center mt-1">
            {new Date(before.created_at).toLocaleDateString('ko-KR')}
          </div>
        </div>
        {afters.map((after) => (
          <div key={after.id}>
            <div className="text-base text-blue-600 mb-1 text-center font-medium">
        ...
        {afters.length === 0 && (
          <div className="col-span-3 flex items-center justify-center bg-gray-50 rounded-lg h-24">
            <p className="text-base text-gray-400 text-center px-4">

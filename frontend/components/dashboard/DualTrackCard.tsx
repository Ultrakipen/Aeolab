...
interface DualTrackCardProps {
  track1Score: number;          // 네이버 AI 브리핑 준비도 (0~100)
  track2Score: number;          // 글로벌 AI 가시성 (0~100)
  ...
  growthStageLabel: string;     // "시작 단계"|"성장 중"|"두각 단계"|"선도 단계"
...
const CATEGORY_MESSAGES: Record<string, { track1Tip: string; track2Tip: string }> = {
  restaurant: {
    ...
    track2Tip: "구글 비즈니스 프로필 등록 + LocalBusiness JSON-LD",
  },
  cafe: {
  ...
  shopping: {
    ...
    track2Tip: "ChatGPT 쇼핑 추천 JSON-LD 적용",
  },
};

const DEFAULT_MESSAGE = {
  track1Tip: "스마트플레이스 FAQ 등록 + 소개글 작성",
  track2Tip: "구글 비즈니스 프로필 등록 + 웹사이트 JSON-LD 적용",
};
    ...
    <div className={`rounded-xl p-3 md:p-4 ${isWeak ? "ring-2 ring-red-400 bg-red-50" : "bg-gray-50"}`}>
      <div className="flex items-start justify-between gap-2 mb-1">
        <div className="flex-1 min-w-0">
          ...
          <span className="text-base text-gray-500 bg-gray-200 rounded-full px-2 py-0.5 inline-block mt-0.5">
            손님 10명 중 {Math.round(pct / 10)}명 경로
          </span>
        </div>
...
export default function DualTrackCard({
  ...
  const isTrack1Weak = track1Score < track2Score - 10;
  const isTrack2Weak = track2Score < track1Score - 10;

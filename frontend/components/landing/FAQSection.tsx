...
const FAQS = [
  ...
  },
  {
    ...
    a: "결과를 100% 보장하지는 않습니다. AI 추천은 가게 정보의 품질, 경쟁사 현황, 각 AI 플랫폼의 알고리즘에 따라 달라지기 때문입니다. AEOlab은 '점수를 높이는 프로세스'를 제공하며, 어떤 점수에서 시작해 어떻게 변화하는지를 측정 데이터로 직접 보여드립니다. 네이버 관련 점수는 2~4주 안에 변화가 나타나며, ChatGPT·Gemini 점수는 수개월이 소요될 수 있습니다.",
  },
  {
  ...
  },
  {
    ...
    a: "ChatGPT는 두 가지 방식으로 가게를 인식합니다. ① 로컬 검색 시 Bing을 실시간으로 검색하므로 Bing Places 비즈니스 등록(무료)이 가장 빠른 노출 경로입니다. ② 학습된 데이터도 참조하므로 구글 비즈니스·외부 블로그·웹사이트에 가게 정보가 충분할수록 인식률이 높아집니다. AEOlab은 지금 내 가게가 실제로 언급되는지를 50~100회 질의해서 측정합니다.",
  },
  {
    q: "효과가 나타나는 데 얼마나 걸리나요?",
    a: "네이버 관련 점수(스마트플레이스·AI 브리핑·AI탭)는 개선 조치 후 2~4주 안에 변화가 나타납니다. ChatGPT는 Bing Places 등록·구글 비즈니스 최적화 후 수일~수주 내 반영되고, 블로그·SNS 콘텐츠 변화는 인덱싱 주기에 따라 수주~수개월이 소요될 수 있습니다. Gemini는 구글 생태계 업데이트 주기에 따라 수주~수개월입니다. AEOlab은 7일 후 자동 재측정으로 변화를 추적하고 알려줍니다.",
  },
  {
    q: "작은 동네 가게도 효과가 있나요?",

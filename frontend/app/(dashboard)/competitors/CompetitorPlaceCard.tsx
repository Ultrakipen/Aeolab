    ...
    <div className="mt-3 bg-white border border-gray-100 rounded-xl p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {competitor.naver_place_id && (
            <a
              href={`https://map.naver.com/p/entry/place/${competitor.naver_place_id}`}
              target="_blank"
      ...
      {competitor.place_has_faq && (
        <div>
          ...
          {competitor.faq_questions?.[0] === '__deprecated__' ? (
            <p className="text-sm text-gray-400 bg-gray-50 rounded-lg px-3 py-2 border border-dashed border-gray-200 leading-relaxed">
              네이버 Q&A 탭이 폐기(2026-05)되어 질문 수집이 불가합니다.
              소개글의 &apos;자주 묻는 질문&apos; 섹션은 위 <strong>소개글 Q&A 섹션 포함 ✓</strong> 표시로 확인하세요.
            </p>
            ...
            >
              ...
              {isFetchingFaq ? '수집 중...' : '경쟁사 소개글 Q&A 확인하기'}
            </button>
          ) : (
            <div className="space-y-1.5">
              ...
              <p className="text-sm text-gray-400 mt-2">위 질문들을 참고해 내 가게 소개글에 Q&A 섹션을 넣으면 AI 검색 노출에 유리합니다.</p>
            </div>
          )}
        </div>
      )}
          ...
          <div className="space-y-3">
            ...
            {(competitor.comp_keywords.missing?.length ?? 0) > 0 && (
              <div>
                ...
                <p className="text-sm text-red-500 mb-1.5">경쟁사 리뷰에는 있지만 우리 가게 리뷰에서 확인되지 않은 키워드입니다. 소개글이나 메뉴에 추가해 보세요.</p>
                <div className="flex flex-wrap gap-1.5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            ...
            title="고객이 네이버 블로그에 자주 언급한 불만 키워드를 분석합니다"
          >
            블로그 분석
          </span>
        </div>
        ...
        {/* 최근 블로그 포스팅 제목 */}
        {competitor.weakness_data?.recent_posts && competitor.weakness_data.recent_posts.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-sm font-medium text-gray-500 mb-2">경쟁사 최근 포스팅</p>
            <div className="space-y-1.5">
              {competitor.weakness_data.recent_posts.map((post, i) => (
                <a
                  key={i}
                  href={post.link}
                  target="_blank"
                ...
                >
                  ...
                  {post.pubDate && (
                    <span className="text-sm text-gray-400 shrink-0 whitespace-nowrap">
                      {new Date(post.pubDate).toLocaleDateString("ko-KR", { month: "short", day: "numeric" })}
                    </span>
                  )}
                </a>
              ))}
            </div>
            <p className="text-sm text-gray-400 mt-2">경쟁사가 최근 올린 포스팅입니다. 어떤 콘텐츠를 활용하는지 참고하세요.</p>
          </div>
        )}
      </div>
    </div>

        ...
        {displayCategories.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[11px] font-semibold text-center leading-tight transition-colors duration-200 ${selected ? colors.icon : "text-gray-600 group-hover:text-gray-800"}`}>
                {cat.label}
              </span>
            </button>
          );
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="block text-sm font-semibold text-gray-800">
          핵심 키워드
          <span className="ml-1.5 text-xs font-normal text-gray-400">AI가 이 키워드로 검색합니다</span>
        </label>
        {keywords.length > 0 && (
          <span className="text-xs text-blue-600 font-medium">{keywords.length}개 등록됨</span>
        )}
      </div>
      ...
      <p className="text-xs text-gray-400 -mt-1">Enter 또는 쉼표로 추가 · 칩의 ×로 삭제 · Backspace로 마지막 삭제</p>

      {/* 업종별 추천 키워드 */}
      {suggestions.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-500 mb-1.5">추천 키워드 (클릭하여 추가)</p>
          <div className="flex flex-wrap gap-1.5">
            {suggestions.slice(0, 10).map((s) => (
              <button
                ...
                className="px-2.5 py-1 text-xs rounded-full border border-gray-200 text-gray-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              >
                + {s}
              </button>
            ))}
      ...
      {keywords.length > 0 && (
        <div className="bg-slate-800 rounded-xl px-4 py-3">
          <p className="text-xs text-slate-400 mb-1.5">손님이 AI에게 물어봤을 때 이렇게 검색됩니다</p>
          <div className="space-y-1">
            ...
            {keywords.length > 3 && (
              <p className="text-xs text-slate-500">+ {keywords.length - 3}개 키워드 더 검색됩니다</p>
            )}
          </div>
        </div>
      )}
...
export function BusinessManager({ businesses, userId, autoEdit }: Props) {
  const router = useRouter();
  const [editingId, setEditingId] = useState<string | null>(
    autoEdit && businesses.length > 0 ? businesses[0].id : null
  );
  ...
  const [editForm, setEditForm] = useState<Omit<Business, "id" | "created_at">>(() => {
    if (autoEdit && firstBiz) {
      return {
        name: firstBiz.name, category: firstBiz.category, region: firstBiz.region,
        address: firstBiz.address ?? "", phone: firstBiz.phone ?? "",
        website_url: firstBiz.website_url ?? "", keywords: firstBiz.keywords ?? [],
        receipt_review_count: firstBiz.receipt_review_count ?? 0,
        visitor_review_count: firstBiz.visitor_review_count ?? 0,
        avg_rating: firstBiz.avg_rating ?? 0, naver_place_id: firstBiz.naver_place_id ?? "",
        is_smart_place: firstBiz.is_smart_place ?? false,
        has_faq: firstBiz.has_faq ?? false, has_recent_post: firstBiz.has_recent_post ?? false,
        has_intro: firstBiz.has_intro ?? false, review_sample: firstBiz.review_sample ?? "",
        blog_url: firstBiz.blog_url ?? "",
      };
    }
    return {
      ...
      naver_place_id: "", is_smart_place: false, has_faq: false, has_recent_post: false,
      has_intro: false, review_sample: "", blog_url: "",
    };
  });
  ...
  return (
    <div className="space-y-3">
      ...
      {businesses.map((biz) => (
        <div key={biz.id} className="border border-gray-100 rounded-xl overflow-hidden shadow-sm">
          {/* 사업장 헤더 */}
          <div className="flex items-center justify-between px-4 py-4 bg-white">
                ...
                return (
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${colors.bg}`}>
                    {IconComp
                      ...
                      : <span className="text-lg">{biz.category.slice(0, 1).toUpperCase()}</span>
                    }
                  </div>
                );
              })()}
              <div className="min-w-0">
                <div className="text-base font-semibold text-gray-900 truncate">{biz.name}</div>
                <div className="text-sm text-gray-400 mt-0.5 truncate">
                  {biz.region} · {getCategoryLabel(biz.category)} · {formatDate(biz.created_at)}
                </div>
              </div>
            </div>
            <div className="flex gap-2 shrink-0 ml-2">
              ...
              <button
                ...
                className="text-sm px-3 py-1.5 border border-red-100 text-red-500 rounded-lg hover:bg-red-50 transition-colors"
              >
                삭제
              </button>
            </div>
          ...
          {/* 수정 폼 */}
          {editingId === biz.id && (
            <div id="biz-edit-section" className="border-t border-gray-100 bg-gray-50 px-4 py-5 space-y-4">
              {/* 사업장 이름 */}
              <div>
              ...
              <div className="bg-gray-100 rounded-xl px-4 py-3 space-y-2">
                ...
                <p className="text-sm text-gray-500">
                  네이버 지도에서 내 사업장 URL의 숫자 부분
                  <span className="block mt-0.5 text-gray-400">예: place.naver.com/place/<strong className="text-gray-600">12345678</strong> → 12345678</span>
                </p>
              </div>
              {/* ── 핵심 키워드 편집 ─────────────────────────────── */}
              <KeywordEditor
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 space-y-3">
                <div>
                  ...
                  <div className="text-xs text-amber-700 mt-0.5">리뷰 수·별점이 AI 노출 점수(리뷰 품질 20%)에 반영됩니다</div>
                </div>

                {/* 확인 방법 안내 */}
                <div className="bg-white border border-amber-200 rounded-lg px-3 py-2.5 space-y-1.5">
                  <div className="text-xs font-semibold text-amber-800">📍 리뷰 수 확인 방법</div>
                  <ol className="text-xs text-amber-700 space-y-1 pl-1">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">
                      영수증 리뷰 수
                      <span className="block font-normal text-gray-400 text-xs">네이버페이 결제 후 작성</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">
                      방문자 리뷰 수
                      <span className="block font-normal text-gray-400 text-xs">일반 방문자 작성</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">
                      평균 별점
                      <span className="block font-normal text-gray-400 text-xs">0.0 ~ 5.0</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                ...
                <div className="flex items-center justify-between text-sm flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleSyncReviews(biz.id)}
                    disabled={syncingReviews || !editForm.naver_place_id}
                    title={!editForm.naver_place_id ? "네이버 플레이스 ID를 먼저 등록해주세요" : undefined}
                    className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-green-600 text-white hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    {syncingReviews ? (
                      <>
                ...
                {syncReviewMsg && (
                  <p className={`text-xs mt-1 ${syncReviewMsg.startsWith("✓") ? "text-green-600" : "text-red-500"}`}>
                    {syncReviewMsg}
                  </p>
                )}
              </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 space-y-3">
                ...
                <div className="bg-white border border-green-200 rounded-lg px-3 py-2.5 space-y-1.5">
                  ...
                  <div className="text-sm text-green-700 space-y-1">
                    <p><strong>① FAQ(질문/답변)</strong> — 스마트플레이스 관리 → [소식·Q&A] → Q&A 등록</p>
                    <p><strong>② 최근 소식</strong> — 스마트플레이스 관리 → [소식·Q&A] → 소식 작성 (1개월 이내)</p>
                ...
                <label className="flex items-start gap-3 cursor-pointer group bg-white border border-green-300 rounded-lg px-3 py-3">
                  ...
                  <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded shrink-0">
                    Track 1 기반
                  </span>
                </label>
                ...
                <div className="space-y-2">
                  {([
                    { key: "has_faq", label: "FAQ(질문/답변) 등록 완료", points: "+30점", desc: "AI 브리핑 직접 인용 경로" },
                    { key: "has_recent_post", label: "최근 소식 등록됨 (1개월 내)", points: "+20점", desc: "최신성 점수 유지" },
                    { key: "has_intro", label: "소개글 작성됨", points: "+10점", desc: "기본 정보 완성도" },
                    ...
                    <label key={key} className="flex items-start gap-3 cursor-pointer group">
                      ...
                      <span className="text-xs font-semibold text-green-600 bg-green-100 px-1.5 py-0.5 rounded shrink-0">
                        {points}
                      </span>
                    </label>
                  ))}
                ...
                <div className="flex items-center justify-between text-sm border-t border-green-100 pt-2">
                  <span className="text-green-700">
                    현재 추가 점수: <strong>{(editForm.has_faq ? 30 : 0) + (editForm.has_recent_post ? 20 : 0) + (editForm.has_intro ? 10 : 0)}점</strong>
                  </span>
                  <button
                    type="button"
                    onClick={() => handleSyncSmartplace(biz.id)}
                ...
                {!editForm.naver_place_id && (
                  <p className="text-xs text-amber-600 mt-1">네이버 플레이스 ID를 먼저 등록해야 자동 확인이 가능합니다.</p>
                )}
                {syncSmartplaceMsg ? (
                  <p className={`text-xs mt-1 ${syncSmartplaceMsg.startsWith("✓") ? "text-green-600" : "text-red-500"}`}>
                    {syncSmartplaceMsg}
                    {syncSmartplaceMsg.startsWith("✓") && " 결과가 실제와 다를 수 있으니 체크 항목을 직접 확인해 주세요."}
                  </p>
                ) : (
                  <p className="text-xs text-gray-400 mt-1">자동 확인은 1시간에 1회 가능 · 결과가 실제와 다를 수 있으니 반드시 직접 확인 후 수정하세요.</p>
                )}
              </div>

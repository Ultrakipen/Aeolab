...
const AI_FAQS = [
  {
    ...
    a: "네이버 AI 브리핑('플레이스형') 노출 대상 업종은 음식점·카페·베이커리·바·숙박입니다. 미용·네일·피부관리·마사지·스파·피트니스·요가·반려동물·약국·댄스·발레·반영구 등 업종은 AI 브리핑 LIKELY(확대 검토 중) 상태로, 현재 AI 브리핑 노출 여부가 불확실합니다. 단, 네이버 AI탭은 업종 제한 없이 모든 사업장이 노출 대상입니다. 프랜차이즈 사업장은 업종에 상관없이 네이버 정책상 '플레이스형' AI 브리핑 대상에서 제외됩니다. 단, '정보형 AI 브리핑'과 AI탭·ChatGPT·Gemini에서는 노출 가능합니다. 그 외 업종(학원·병원·법무사·부동산 등)은 '플레이스형' AI 브리핑 대상이 아니지만, 블로그·콘텐츠가 잘 갖춰지면 '정보형 AI 브리핑'에 노출될 수 있습니다. ChatGPT·Gemini·Google AI 노출 최적화도 효과적이며, AEOlab은 이 채널도 함께 측정합니다. 정확한 업종 여부는 무료 체험에서 바로 확인할 수 있습니다.",
  },
  {
    q: "네이버 AI탭과 AI 브리핑은 어떻게 다른가요?",
    a: "AI 브리핑은 검색 결과 상단에 자동으로 뜨는 추천 박스입니다. 그 중 가게를 요약하는 '플레이스형'은 음식점·카페·숙박 등 일부 업종만 해당하지만, 블로그·콘텐츠가 출처로 채택되는 '정보형 AI 브리핑'은 업종 제한이 없어 다른 업종도 노출될 수 있습니다. AI탭은 검색창 상단 'AI' 탭을 클릭하면 나오는 화면으로, 업종 제한 없이 모든 사업장이 노출될 수 있습니다. (2026-04-27 베타 출시 · 2026-06-25 정식 출시됨)",
  },
  {
    q: "ChatGPT는 어떤 기준으로 가게를 추천하나요?",
    a: "실사용자의 ChatGPT는 웹검색 기능이 켜져 있으면 Bing 기반으로 실시간 답변하고, 꺼져 있으면 학습 데이터 기반으로 답변합니다. 웹검색 활성 시에는 구글 비즈니스 프로필·자체 웹사이트·트립어드바이저 등 Bing이 인덱싱하는 콘텐츠를 참조합니다. 네이버 블로그·스마트플레이스는 Bing 인덱싱이 제한적이어서 ChatGPT 웹검색에 직접 영향이 없습니다. AEOlab의 ChatGPT 스캐너는 AI 학습 데이터 기반으로 측정하며, 실시간 웹검색 결과와 다를 수 있습니다.",
  },
  {
  ...
  },
  {
    ...
    a: "채널마다 반영 기간이 다릅니다. 네이버 AI 브리핑·AI탭: 소개글·소식 업데이트 후 2~4주. Gemini·ChatGPT: AEOlab 스캐너 점수는 둘 다 학습 데이터 기반이므로 모델 재학습 주기(수개월~1년)에 의존합니다. 실사용 Gemini 앱은 구글 검색 그라운딩 기반이라 구글 비즈니스 프로필 갱신 후 수 주~수개월 내 더 빠르게 반영될 수 있지만, 이는 AEOlab이 측정하는 스캐너 점수와는 별개입니다.",
  },
  {
    q: "Google AI 스캔은 어떻게 진행되나요?",
    a: "Google AI Overview는 실제 Google 검색 결과를 조회해 측정합니다. 스캔 쿼리별로 AI Overview 영역에 내 사업장이 포함되는지 직접 확인하며, 안정적으로 측정되고 CAPTCHA 차단 없이 정상 작동합니다.",
  },
  {
  ...
  },
  {
    ...
    a: "Google의 Gemini는 Google 비즈니스 프로필(구 구글 마이비즈니스)과 Google 지도 데이터, 그리고 웹 전반의 콘텐츠를 기반으로 답변합니다. 구글 비즈니스 프로필에 등록된 업종·설명·리뷰·사진·영업 시간이 Gemini 답변에 직접 반영됩니다. 네이버 스마트플레이스는 Gemini 답변에 거의 영향을 주지 않습니다. 실사용 Gemini 앱은 구글 검색 그라운딩 기반이라 구글 비즈니스 프로필 업데이트 후 수 주~수개월 내 반영될 수 있습니다. 다만 AEOlab이 Gemini 스캐너로 50~100회 샘플 쿼리를 실행해 측정하는 점수는 학습 데이터 기반이라 반영까지 수개월~1년이 소요됩니다.",
  },
  {
    q: "내 업종은 어떤 AI 채널에 집중해야 하나요?",
    a: "업종별 권장 채널이 다릅니다. ▪ 음식점·카페·베이커리·주점·숙박: '플레이스형' 네이버 AI 브리핑 + AI탭이 핵심. 스마트플레이스 완성도·리뷰·블로그가 최우선. ▪ 미용·네일·피부관리·마사지·스파·피트니스·요가·반려동물·약국·댄스·발레·반영구 등: AI탭(모든 업종 가능) + ChatGPT·Gemini 병행. '플레이스형' AI 브리핑은 확대 예정이나 현재 불확실. ▪ 학원·병원·법무사·부동산·인테리어 등: 네이버 플레이스 일반 검색 상위노출 + ChatGPT·Gemini 위주. 단, 세 그룹 모두 블로그·콘텐츠가 잘 갖춰지면 업종 제한 없는 '정보형 AI 브리핑'에는 노출될 수 있습니다. 구글 비즈니스 프로필 등록이 특히 중요. AEOlab 진단을 받으면 내 업종·지역의 경쟁사가 어느 채널에서 노출 중인지와 내가 부족한 채널을 함께 확인할 수 있습니다.",
  },
];
...
const SERVICE_FAQS = [
  {
    ...
    a: "AEOlab은 네이버 점수와 글로벌 AI 점수를 업종별 비중에 맞게 합산합니다. 네이버 점수는 스마트플레이스 완성도, 네이버 생태계 활동, 리뷰 수, 키워드 커버리지를 측정하고, 글로벌 AI 점수는 Gemini·ChatGPT 언급률, Google AI Overview 노출 여부, 웹사이트 정보 구조, 콘텐츠 품질을 평가합니다. 업종마다 기준 비중이 다릅니다. 예를 들어 음식점은 네이버 80%, 법률 서비스는 글로벌 AI 80% 비중입니다.",
  },
  {
    q: "무료 체험과 유료 구독의 차이는?",
    a: "무료 체험은 업종·지역을 입력하면 경쟁사 현황과 기본 점수를 한 번 확인할 수 있습니다. 유료 구독(Basic 11,900원/월)은 내 사업장을 등록하고, AI 자동 스캔(네이버·ChatGPT·Gemini·Google AI Overview 4채널) + 경쟁사 비교, 30일 트렌드, 월별 개선 가이드를 이용할 수 있습니다. 첫 달은 5,950원으로 시작할 수 있습니다.",
  },
  {
  ...
  {
    q: "언제든지 해지할 수 있나요?",
    a: "네, 언제든지 해지할 수 있습니다. 구독 설정 페이지에서 직접 해지하면 다음 결제일부터 청구되지 않습니다. 남은 기간은 그대로 이용할 수 있으며, 위약금이나 추가 비용은 없습니다. 토스페이먼츠 카드 자동결제 방식으로 매월 결제됩니다(카드 최초 1회 등록 필요).",
  },
];
  ...
  return (
    <details className="border border-gray-200 rounded-xl overflow-hidden group">
      <summary className="px-5 py-4 bg-white hover:bg-gray-50 transition-colors cursor-pointer list-none flex items-start gap-3">
        ...
        <span className="shrink-0 ml-2 text-gray-600 group-open:rotate-180 transition-transform duration-200" aria-hidden="true">

...
const AI_FAQS = [
  {
    ...
    a: "네이버 AI 브리핑 네이버 플레이스 노출 대상 업종은 음식점·카페·베이커리·주점·숙박입니다. 미용·네일·피트니스·요가·반려동물 업종은 2026년 AI 탭 베타 확대 진행 중입니다. 프랜차이즈 사업장은 업종에 상관없이 네이버 정책상 AI 브리핑 대상에서 제외됩니다. 그 외 업종(학원·병원·법무사·부동산 등)은 ChatGPT·Gemini·Google AI 노출 최적화가 더 효과적이며, AEOlab은 이 채널도 함께 측정합니다. 정확한 업종 여부는 무료 체험에서 바로 확인할 수 있습니다.",
  },
  {
    q: "네이버 AI탭과 AI 브리핑은 어떻게 다른가요?",
    a: "AI 브리핑은 검색 결과 상단에 자동으로 뜨는 추천 박스로, 음식점·카페·숙박 등 일부 업종만 해당합니다. AI탭은 검색창 상단 'AI' 탭을 클릭하면 나오는 화면으로, 업종 제한 없이 모든 사업장이 노출될 수 있습니다. (2026년 4월 베타 출시, 전체 확대 예정)",
  },
  {
    q: "ChatGPT는 어떤 기준으로 가게를 추천하나요?",
    a: "실사용자의 ChatGPT는 Bing 웹검색 기반으로 답변합니다. 구글 비즈니스 프로필·자체 웹사이트·트립어드바이저 등 Bing이 인덱싱하는 콘텐츠를 참조합니다. 네이버 블로그·스마트플레이스는 Bing 인덱싱이 제한적이어서 ChatGPT 웹검색에 직접 영향이 없습니다. AEOlab의 ChatGPT 스캐너는 AI 학습 데이터(컷오프 2024.06) 기준으로 측정하며, 실시간 웹검색 결과와 다를 수 있습니다.",
  },
  {
  ...
  },
  {
    ...
    a: "채널마다 반영 기간이 다릅니다. 네이버 AI 브리핑·AI탭: 소개글·소식 업데이트 후 2~4주. Gemini: 구글 비즈니스 프로필 업데이트 후 수주 내 반영 (Google Search 그라운딩으로 빠름). ChatGPT: AEOlab 스캐너 점수는 ChatGPT 학습 데이터 기반이므로 모델 재학습 주기(수개월~1년)에 의존합니다. 실사용자 ChatGPT 웹검색(Bing 기반)은 구글 비즈니스 프로필 등록 후 1~4주 내 반영됩니다.",
  },
  {
...
const SERVICE_FAQS = [
  ...
  {
    q: "무료 체험과 유료 구독의 차이는?",
    a: "무료 체험은 업종·지역을 입력하면 경쟁사 현황과 기본 점수를 한 번 확인할 수 있습니다. 유료 구독(Basic 9,900원/월)은 내 사업장을 등록하고, AI 3종 자동 스캔(네이버·ChatGPT·Gemini) + Google AI(현재 CAPTCHA 차단으로 일시 제한, 자동 재배분 처리), 경쟁사 비교, 60일 트렌드, 월별 개선 가이드를 이용할 수 있습니다. 첫 달은 4,950원으로 시작할 수 있습니다.",
  },
  {

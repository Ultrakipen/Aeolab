...
export function LandingLogout({ email }: { email: string }) {
  ...
  return (
    <div className="flex items-center gap-3 shrink-0">
      <Link href="/settings" title={email} className="text-sm text-gray-600 hover:text-blue-600 hidden sm:block transition-colors whitespace-nowrap max-w-[220px] truncate">{email}</Link>
      <button

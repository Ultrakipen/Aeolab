    ...
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">관리자 감사 로그 · 시스템 알림 이력</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
        ...
        {loading ? (
          <div className="text-sm text-gray-600 py-4">불러오는 중...</div>
        ) : alerts.length === 0 ? (
          <div className="text-sm text-gray-600 py-4">아직 기록된 알림이 없습니다.</div>
        ) : (
            ...
            {alerts.map((a) => (
              <div key={a.id} className="border border-gray-100 rounded-lg p-3">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  {a.source && <span className="text-sm text-gray-600">{a.source}</span>}
                  <span className="text-sm text-gray-600 ml-auto">{formatDate(a.created_at)}</span>
                </div>
                <p className="text-sm font-medium text-gray-900">{a.subject}</p>
                {a.message && <p className="text-sm text-gray-600 mt-0.5 whitespace-pre-wrap line-clamp-3">{a.message}</p>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">2026-07-10 이후 이벤트만 기록됩니다(소급 이력 없음). 최초 결제·자동 갱신 재시도를 성공/실패와 함께 기록합니다.</p>
        {loading ? (
          <div className="text-sm text-gray-600 py-4">불러오는 중...</div>
        ) : paymentEvents.length === 0 ? (
          <div className="text-sm text-gray-600 py-4">아직 기록된 결제 이벤트가 없습니다.</div>
        ) : (
          <div className="space-y-2 max-h-[420px] overflow-y-auto">
            {paymentEvents.map((p) => (
              <div key={p.id} className="border border-gray-100 rounded-lg p-3">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="text-sm text-gray-600">{EVENT_TYPE_LABEL[p.event_type] ?? p.event_type}</span>
                  {p.amount != null && <span className="text-sm text-gray-600">{p.amount.toLocaleString()}원</span>}
                  <span className="text-sm text-gray-600 ml-auto">{formatDate(p.created_at)}</span>
                </div>
                <p className="text-sm text-gray-700">{p.email ?? p.user_id}</p>
                {p.status === "failed" && p.detail && (
                  <p className="text-sm text-red-700 mt-0.5 whitespace-pre-wrap line-clamp-2">{p.detail}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">2026-07-10 이후 요청만 기록됩니다(소급 이력 없음). 사업장 미등록(예비 창업자)이면 &quot;미등록&quot;으로 표시됩니다.</p>
        {startupReports.length === 0 ? (
          <div className="text-sm text-gray-600 py-2">아직 기록된 창업리포트 요청이 없습니다.</div>
        ) : (
          <div className="space-y-2 max-h-[420px] overflow-y-auto">
            {startupReports.map((r) => (
              <div key={r.id} className="border border-gray-100 rounded-lg p-3">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  {!r.business_id && (
                    <span className="text-sm text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">미등록(예비 창업자)</span>
                  )}
                  <span className="text-sm text-gray-600 ml-auto">{formatDate(r.created_at)}</span>
                </div>
                <p className="text-sm text-gray-600">{r.email ?? r.user_id}{r.business_name ? ` · ${r.business_name}` : ""}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">owner는 구독 강제해지/환불까지 전체 권한, support는 조회·콘텐츠 관리만 가능(금전이동 불가). owner 계정으로 로그인해야 이 섹션을 관리할 수 있습니다.</p>
        ...
        {adminUsers.length === 0 && !adminUsersError ? (
          <div className="text-sm text-gray-600 py-2">등록된 관리자 계정이 없습니다.</div>
        ) : (
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        ...
        <p className="text-sm text-gray-600 mb-4">POST·PATCH·DELETE 관리자 액션만 기록됩니다(조회는 제외). admin_email이 비어있으면 curl 등 직접 호출입니다.</p>
        {loading ? (
          <div className="text-sm text-gray-600 py-4">불러오는 중...</div>
        ) : auditLog.length === 0 ? (
          <div className="text-sm text-gray-600 py-4">아직 기록된 관리자 액션이 없습니다.</div>
        ) : (
          <>
            {/* PC 테이블 */}
                ...
                <tbody>
                  {auditLog.map((row) => (
                    <tr key={row.id} className="border-b border-gray-50 align-top">
                      <td className="py-2 pr-3 text-gray-600 whitespace-nowrap">{formatDate(row.created_at)}</td>
                      ...
                      <td className="py-2 text-gray-600">{row.status_code}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

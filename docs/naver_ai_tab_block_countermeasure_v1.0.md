...
> ⚠️ **2026-07-04 갱신** — 07-02/07-03 재차단 발생 + 근본 원인 확정. §0.1 먼저 읽을 것.

---

## §0.1. 2026-07-04 최종 결론 — 근본 원인은 `block_heavy_resources`였음 (IP 아니었음)

**07-02/07-03 실제 고객 배치 스캔에서 100% 재차단 발생 → 조사 끝에 확정된 진짜 원인:**

`naver_ai_tab_scanner.py`의 `ctx.route("**/*", block_heavy_resources)`(이미지/미디어/폰트 100% 차단, 대역폭 절약용으로 도입됐던 코드)가 네이버 AI탭 전용 봇 탐지를 유발했음. 일반 검색 페이지는 같은 차단 상태에서도 문제없었음 — AI탭에만 더 엄격한 탐지가 적용된 것으로 판단.

**검증**: 이 라우트 하나만 켜서 테스트 → 즉시 100% 차단 재현. 빼고 테스트 → 3/3 성공(실제 AI 답변 수신). 실제 운영 함수 `nat.scan()`으로도 제거 후 3/3 성공 재확인.

**조사 중 함께 처리된 부수 개선(원인은 아니었지만 유지 중)**:
- Webshare(고정 IP 8개뿐으로 확인됨, 진짜 로테이팅 아님) → **DataImpulse 종량제 로테이팅 프록시**로 전환($1/GB, 한국 타겟팅 `login__cr.kr`)
- iwinv 서버 아웃바운드 방화벽이 프록시 포트(823)를 막고 있어 콘솔에서 아웃바운드 규칙 추가
- 스캔 딜레이 랜덤화 + 사업장별 스캔 시각 분산 (봇 패턴 완화, 보조적 조치)
- `naver_ai_tab_visible` 측정실패→false 오확정 버그 수정 (별개 버그, UI "측정 불가" 표시 정상화)

**남은 확인**: 2026-07-05 02:00 KST 정식 배치가 수정 이후 첫 검증(§0 참조 시 이 문서보다 메모리 `project_naver_ai_tab_block_countermeasure`가 최신 — 거기 확인).

**교훈**: 차단 원인을 IP/프록시로 단정하지 말 것 — 코드 자체의 자동화 신호(리소스 차단 등)가 원인일 수 있음. 아래 §0(2026-06-28 작성) 이후 섹션은 그 시점 기준 정보이며 일부 이력용으로만 유효.

---

## §0. 현재 상태 (2026-06-28 기준, 이력용 — 최신 아님) — ✅ 측정 정상 작동

| 항목 | 상태 |
|------|------|
| `NAVER_AI_TAB_ENABLED` | `true` |
| `NAVER_PROXY_LIST` | ~~Webshare.io KR 주거용 6개 (galtcbkj-kr-1~6)~~ → **2026-07-04부터 DataImpulse로 교체** |
| 실제 측정 가능 여부 | ✅ **정상 측정 중** |
...
### B단계 — (2026-07-04 기준 불필요해짐 — §0.1 참조, 근본원인이 코드였고 무료로 해결됨. 아래는 이력용 참고)

### B단계 원안 — 구독자 50명 이후: 실제 브라우저 클라우드 서비스

...
export default async function LandingPage() {
  ...
  return (
    <main className="min-h-screen" style={{ background: "#fafaf8" }}>
      ...
      <header className="border-b px-4 md:px-6 py-3 md:py-4 sticky top-0 z-50 backdrop-blur"
        style={{ background: "rgba(250,250,248,0.96)", borderColor: "#e8e4dc" }}>
      ...
      {/* ══ 블록 1: 히어로 ══ */}
      <div style={{ background: "#f5f0e8", borderBottom: "1px solid #e8e4dc" }}>
        <section className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-14 relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-start">
            <div>
              ...
              <div className="text-sm md:text-base text-gray-700 mb-3 leading-relaxed break-keep bg-white border-l-4 border-blue-300 shadow-sm pl-3 pr-3 py-2.5 rounded-r-lg">
              ...
              <div className="flex flex-col items-start gap-3 mb-2">
                ...
                <div className="hidden sm:flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 text-base text-gray-700 bg-white border border-[#e8e4dc] px-3 py-1.5 rounded-full">
                    ✓ 블로그 인용 분석
                  </span>
                  <span className="inline-flex items-center gap-1 text-base text-gray-700 bg-white border border-[#e8e4dc] px-3 py-1.5 rounded-full">
                    ✓ 스마트플레이스 자동 점검
      ...
      {/* ══ 블록 2: 신뢰 지표 바 ══ */}
      <section className="bg-white border-b py-6 md:py-7 px-4 md:px-6"
        style={{ borderColor: "#e8e4dc" }}>
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-0">
          {[
            { num: "4종", label: "AI 플랫폼 동시 측정", sub: "네이버·ChatGPT·Gemini·Google" },
            { num: "100회", label: "샘플링 기반 통계", sub: "Full 스캔 기준 · 변동성 최소화" },
            { num: "25종", label: "업종 지원", sub: "음식점부터 전문직까지" },
            { num: "7일", label: "만에 개선 시작", sub: "첫 스캔 → 가이드 → 실행" },
          ].map((item, i) => (
            <div key={i} className="text-center px-4 py-2"
              style={{ borderLeft: i > 0 ? "1px solid #e8e4dc" : undefined }}>
              <p className="text-2xl md:text-3xl font-black text-blue-600 leading-none">{item.num}</p>
              <p className="text-sm font-bold text-gray-800 mt-1">{item.label}</p>
              <p className="text-xs text-gray-400 mt-0.5 hidden md:block">{item.sub}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ══ 블록 5: 추가 도구·비교 (DiagnosticTools + AEOvs 통합 그룹) ══ */}
      <section className="border-y py-6 md:py-8 px-4 md:px-6"
        style={{ background: "#fafaf8", borderColor: "#e8e4dc" }}>
      ...
      {/* ══ 블록 7: AEOlab 동작 기준 (게이트 + 점수 + 한계) ══ */}
      <ServiceMechanismSection />

      {/* ══ 블록 8: 후기 (실데이터 시 자동 표시) ══ */}
      <Testimonials />

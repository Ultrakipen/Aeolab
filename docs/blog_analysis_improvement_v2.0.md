# blog-analysis 개선 v2.0 — 검색량 연동 + 잔여 개선안 (2026-07-06, 2026-07-17 §4로 §2-B/C 결론 갱신)

> §1(v1.0 4건)·§2-A(검색량 연동)·§2-B(경쟁사 실시간비교, 완전배제)·§2-C(LLM품질판정, 구현완료) 전부 결론 남 —
> 이 문서 트리거는 완료 이력 확인용. §4가 최신 상태, 잔여 작업은 §4 맨 아래 참조.
> v1.0(`docs/blog_analysis_improvement_v1.0.md`, 2026-07-04)에서 이미 완료된 4건은 §1 참조.
>
> **§2-A(1순위) 완료 (2026-07-06, git `eeb4615`)** — 아래 §2-A 스펙대로 구현 + 배포 + 실측 검증 완료.
> 구현 중 별도 버그 발견·수정: `naver_searchad.py get_keyword_volumes`가 배치에 공백 포함 키워드가
> 하나라도 있으면 HTTP 400으로 배치 전체가 무너지는 문제(업종 키워드 사전에 "주차 가능" 류가 흔해
> 기존 `report.py` 키워드 트렌드 기능에도 이미 영향 중이었음) — 공백 제거 후 조회 + 원본 키워드
> 재매핑으로 근본 수정. 서버에서 실제 API 호출로 단건/배치/캐시 read-write 전부 재현 검증함.
> 라이브 브라우저 클릭 테스트는 로그인 자격 증명 없어 미실시 — 백엔드 파이프라인은 실제 데이터로 완전 검증됨.
>
> **후속 작업 완료 (2026-07-06, git `cb9be7f`)** — 검색량 배지 설명 문구를 같은 공백이 확인된
> `GuideClient.tsx`(개선 가이드 "내가 놓친 키워드" 카드)·`KeywordTrendChart.tsx`(대시보드 "키워드
> 검색 트렌드" 카드)에도 동일 적용. `blog.py`가 SearchAd에서 이미 받아오는 `competition`(경쟁도:
> high/medium/low) 필드를 버리고 있던 것을 발견해 병합, "경쟁 낮음/보통/높음" 배지 + 경쟁 낮음·검색량
> 있음 조합엔 "선점 기회" 힌트 추가. (오판 정정: `KeywordRankCard.tsx`도 같은 공백이 있다고 봤던 건
> 재검증 결과 틀렸음 — `monthly_volume` 필드가 타입에만 선언되고 실제 렌더링 지점이 없어 제외)
>
> **"블로그 진단" 페이지 측정 감사 완료 (2026-07-06, git `a0b78bd`)** — `docs/nine_pages_measurement_
> inspection_v1.0.md` §3.6 방법론으로 Layer A(프론트 UI)/Layer B(백엔드 파이프라인) 병렬 감사 →
> P0 1건 + P1/P2 6건 확정·수정·배포. 가장 심각했던 건 네이버 API/RSS가 전체 실패해도 "포스트 0개"로
> 반환돼 "사업장명/블로그 주소를 확인하세요"라는 오도성 안내가 뜨던 버그. 상세는 메모리
> `project_blog_analysis_measurement_audit_2026_07_06` 참조. Layer B 보고 6건 중 3건은 실제로는
> "변화 기록"(history) 페이지 전용 코드로 확인돼 이번 스코프에서 제외함(스코프 오판 자기정정 사례).
>
> **§2-A 라이브 검증 + 재점검 2차 완료 (2026-07-06, git `31af359`)** — 로그인 자격 증명이 없어
> 미실시였던 라이브 브라우저 테스트를 Playwright(admin 계정)로 실제 운영 서버에서 실행, "월 XXX회"
> 배지·경쟁도 라벨·내림차순 정렬 전부 스펙대로 정상 노출 확인. 이 과정에서 같은 블로그를 90초 내
> 3회 재분석했더니 RSS가 5→5→0개로 들쭉날쭉한 것을 실측(pm2 로그 대조) → Layer A/B 재점검 착수해
> [P0] 1건(최초 P1로 오분류 후 정정 — commercial_inspection_standard_v2.0 §4 "기능 오작동" 기준)
> + [P2] 8건 확정·수정·배포. 가장 심각했던 건 RSS만 실패하고 API 쿼리가 우연히 0건
> 매칭이면 "전체 실패" 조건(실패율=총시도)을 통과 못 해 `error=None`으로 빠져 **blog.py가 이를
> 성공으로 오인해 기존 `blog_post_count`를 0으로 덮어쓰고 24시간 쿨다운을 리셋 + 월 사용량을
> 소비시키던 버그**(a0b78bd가 고친 "전체 실패" 케이스와 형태는 같으나 "부분 실패+우연한 0건"은
> 사각지대였음). RSS 재시도 1회 추가, `rss_failed` 플래그 신설, 프론트 5건(플랜 무관 업그레이드
> 문구·추이차트 오분류·첫분석 새로고침 안내 오류·priorityLabel 구조 취약점·PC suggestion 누락)도
> 같이 수정. 상세는 메모리 `project_blog_analysis_v2_reaudit_2026_07_06` 참조. §2-B/C/D는 활성
> 구독자 5명·`blog_score_history` 2행·서버 미업그레이드로 재확인 결과 여전히 트리거 미충족.
...
## 4. 2026-07-17 세션 갱신 — §2-B 완전 배제 확정 + §2-C 구현 완료

> naeo.kr 대응 블로그 진단 개편(§2-B/§2-C 작성 이후 별도 세션, git `db5bc14`~`47de959`)에서 §2-B/§2-C 둘 다 결론이 남. 이 문서의 §2-B/§2-C 절은 위 내용을 히스토리로 남기고, 최종 결론은 여기 기재.

### §2-B — "후순위 보류"가 아니라 **완전 배제로 확정**

사용자 확인: "경쟁사 실시간 구조비교는 제외할 것, 사용자의 블로그가 AI에 최적으로 노출되고 있는지가 가장 중요함."

재검증 결과 §2-B의 원래 전제("저장된 점수를 실시간으로 바꾸자")도 틀렸던 것으로 확인됨 — `competitors.blog_analysis_json` 컬럼이 애초에 존재한 적이 없어(`f5ce0c8`) `competitor_blogs`는 코드상 항상 빈 리스트였음. 즉 병목은 "실시간이 아니라서"가 아니라 **"경쟁사 블로그 URL 자체를 모른다"**는 데이터 소스 문제 — 경쟁사는 네이버 플레이스에서 수집돼 블로그 등록 절차가 없음. 서버 업그레이드로도 안 풀리는 문제라 트리거 재설정 자체가 무의미 — **재검토 대상에서 제외**. 죽은 점수·순위 비교 코드(`avg_score`/`my_rank`/`competitors`/`score_comparison_available`) 전체 제거, 중복 노출되던 프론트 섹션도 삭제(`eaec229`, `74ac494`).

### §2-C — 구현 완료 (BEP 트리거 조건 예외 적용)

사용자가 명시적으로 BEP(구독자 20명) 미달 상태에서도 진행을 승인 — 호출당 비용이 수 원 수준(단일 호출, 상위 5개 일괄 판정)이라 트리거 예외로 처리. `backend/services/blog_quality_scorer.py` 신설, Claude Haiku로 "진솔한 경험담 vs 홍보 문구" 판정, 결과는 기존 `positives`/`issues` 배열에 "AI 판단:" 접두사로 추가(신규 UI 컴포넌트 없음). **네이버 경로만 적용** — 외부 블로그(티스토리/워드프레스)는 desc 텍스트가 없어(제목만 존재) 신뢰도 낮은 판단이라 제외(사용자 확인: "티스토리/워드프레스 파싱 추가 투자 불필요" — 활성 사업장 5곳 중 블로그 등록 2곳 모두 네이버, 실사용자 0명). 배포 검증: 서버에서 Claude API 직접 호출로 genuine=True/False 정확히 구분 확인, 도중 Anthropic 529 과부하 실측(재시도로 해결, 코드 문제 아님). git `ddb9a62`.

### 같은 세션에서 함께 처리된 것 (§2 범위 밖, 참고)

- 채널 스코프 정밀성(네이버 전용 인용 배지를 "AI"로 뭉뚱그림, 4채널을 5채널로 오기재) 4곳 수정 — `b2b5b3c`
- `get_blog_result` citation_benchmark 전용 TTL 캐시(다른 라이브 데이터는 그대로 유지) — `eec9807`
- RSS·검색 API 429(rate limit) 재시도 추가 — `47de959`
- pytest 최초 도입 + 회귀 테스트 신설 — 실행 도중 기존 테스트 버그 1건(`ast.AnnAssign` 미검출) + `score_engine.py` 배포 드리프트(music_studio 서버 미반영) 발견·수정 — `47de959`
- AI 인용 체크리스트 8번 라벨 정정, 외부블로그 포스트별 날짜 오귀속 수정, `_generate_title_suggestions()` 업종 미매칭 시 오추천(예: music_studio에 "맛집 추천") 수정 — `ad85d1d`

**남은 것**: CI(GitHub Actions)에 테스트 게이트 미설정, 비동기 함수(`score_posts_quality`, `_get_cached_citation_benchmark`) 테스트 커버리지 없음, 로컬 git이 서버 git보다 여러 커밋 앞서 있어 reconcile 필요(§자세한 내용은 프로젝트 메모리 `project_blog_analysis_rss_retry_and_pytest_2026_07_17` 참조).

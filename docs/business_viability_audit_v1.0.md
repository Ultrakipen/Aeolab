...
### 2026-07-16 재확인 — 카테고리 오류 + 영세 등급 확인으로 전면 재계산

**발견 1 — 4.3%는 잘못된 상품 카테고리였음**: `tosspayments.com/about/fee`를 직접 WebFetch로 재확인한 결과, 4.3%는 토스의 **브랜드페이**(저장카드 원클릭 결제 상품) 수수료였다. AEOlab이 실제 쓰는 결제 흐름(`backend/routers/webhook.py`의 `POST /v1/billing/authorizations/issue`, `POST /v1/billing/{billingKey}`)은 브랜드페이가 아니라 **표준 신용카드 자동결제(빌링키)** API — 이 카테고리의 일반 등급 수수료는 **3.4%+VAT**다(같은 페이지에 "신용·체크카드 일반 3.4%"로 별도 명시).

**발견 2 — 사용자 확인: 영세 등급 해당**: `docs.tosspayments.com/resources/glossary/smm`(토스 공식 문서) 기준 연매출 3억원 이하는 "영세" 등급, 신용카드 수수료 **0.40%**(체크카드 0.15%). 여신금융협회가 매년 1월·7월 국세청 자료로 자동 산정·소급 적용(신청 불필요). **사용자 확인 결과 AEOlab 사업자(개인사업자)는 연매출 3억원 이하 영세 구간에 해당** — 신용카드 0.40%+VAT(10%) = 실효 **0.44%**.

**발견 3 — 가격 인상 반영 누락**: 기존 표는 가격 인상 전 구가격(Basic 9,900원, Pro 18,900원)으로 계산돼 있었음. CLAUDE.md 현재 확정가(Basic 11,900원/창업패키지 12,900원/Pro 23,900원/Biz 49,900원)로 재계산.

### 재계산 결과 (2026-07-16, 영세 등급 0.44% + 현재가 기준)

| 구분 | 매출(현재가) | API 비용(기존 추정, 미검증) | PG 수수료(0.44%, 영세등급) | 총 비용 | **마진율(최종)** |
|---|---:|---:|---:|---:|---:|
| Basic | 11,900원 | 1,876원 | 52원 | 1,928원 | **83.8%** |
| 창업패키지 | 12,900원 | 2,021원 | 57원 | 2,078원 | **83.9%** |
| Pro | 23,900원 | 3,159원 | 105원 | 3,264원 | **86.4%** |
| Biz(1사업장) | 49,900원 | 4,093원 | 220원 | 4,313원 | **91.4%** |

> **참고 — 교정 단계별 효과 분리** (카테고리 정정과 등급 우대를 한 번에 섞지 않기 위한 중간 단계, 현재가 기준 표준카드 일반 3.4%+VAT=3.74%만 적용 시): Basic 80.5% / 창업패키지 80.6% / Pro 83.1% / Biz 88.1%. 여기서 영세 등급(0.44%)까지 적용하면 위 최종 표(83.8~91.4%)가 됨 — 카테고리 정정 자체가 +4~5%p, 영세 등급이 추가로 +3%p 개선.

> **주의**: "API 비용" 열은 여전히 §1에서 확인한 대로 실측이 아닌 가정치(Gemini thinking 토큰 미반영 가능성) — 이번 재계산은 **PG 수수료 축(카테고리+등급, 사용자 확인 완료로 이제 확정값)만 교정**한 것이며, API 비용 축의 불확실성은 `ai_usage_log` 텔레메트리 누적 후(2026-07-26 트리거) 별도 재교정 필요. 두 불확실성을 섞지 말 것.
> 2026-07-12 수치(76.3/79.6/78.6/87.1%)는 PG 수수료를 처음 반영한 것 자체는 옳은 방향이었으나 카테고리(브랜드페이 vs 표준카드)·등급(일반 vs 영세)·가격(구가격) 3가지가 전부 부정확했던 값 — 폐기.

### 조치
- CLAUDE.md "마진율" 줄 교정 완료(2026-07-16)
- `plan_limits_v1.0.md` 포인터 문구 갱신 완료
- `commercial_launch_inspection_status_v1.0.md §6-1` "PG 수수료 우대 등급 확인" 항목 완료 처리
...
1. ~~Gemini SDK 마이그레이션 검토~~ — **2026-07-16 보류 결정**(사용자 확인). 트리거일(14일) 전이지만 누적 데이터(2026-07-12~15, 1,947건, Gemini 541건)를 조기 확인한 결과 `thinking_tokens` 필드는 SDK(0.8.3) 구조적 한계로 **영원히 None**이라 더 기다려도 직접 계측은 불가능함을 재확인. 대신 간접 신호(평균 output 63~65토큰/호출, 최대 211개, 호출당 약 0.25원)로는 thinking이 비용을 부풀리는 정황이 보이지 않음 — 마이그레이션의 원래 근거(thinking 비용 통제)가 약해진 상태에서 scan-engine 리스크를 감수할 실익이 낮다고 판단. **재검토 트리거**: SDK deprecated 공지 등 다른 사유 발생 시

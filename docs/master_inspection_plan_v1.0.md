...
### §6.2 gap-5 실제 스크린샷 비교 완료 (2026-07-11, 같은 세션 — 모든 병렬 세션 중지 후)

- **핵심 돌파구**: `mcp__playwright__*`는 이번에도 동일하게 lock 실패했으나, `browse` 스킬(gstack, `~/.claude/skills/gstack/browse/dist/browse`)이 별도 Chromium 프로세스라 **lock 없이 즉시 성공** — memory `project_p4_support_inspection_2026_07_11`에 이미 기록된 대안을 재확인·재사용. 향후 `mcp__playwright__*`가 lock되면 매번 재시도 대신 바로 `browse` 스킬로 전환할 것(더 이상 "브라우저 락 해제 대기" 상태로 남겨두지 말 것).
- **실제 스크린샷 확보**: `aeolab.co.kr/`(전체 13,660px)·`aeolab.co.kr/pricing`(전체 6,797px)·`i-boss.co.kr/`(전체 3,410px, `useragent` 데스크톱 UA 설정 후 200 OK로 통과 — Cloudflare가 headless 감지 시 JS 챌린지를 걸지만 데스크톱 UA 지정 시 최초 요청은 통과하는 경우가 있음, 재요청 시 챌린지 재발동 확인돼 반복 요청은 불안정·비확정적이므로 추가 크롤링 시도는 자제)
- **시각 비교로 새로 확인된 사실(§6.1의 코드/HTML 구조 비교보다 격상됨)**:
  1. **페이지 길이 격차**: AEOlab 홈 13,660px(≈15스크린) vs i-boss.co.kr 홈 3,410px(≈4스크린) — **약 4배**. i-boss는 한 화면에 정보 밀도를 압축한 포털형, AEOlab은 세로로 긴 스토리텔링형 SaaS 랜딩. 어느 쪽이 "낫다"는 아니지만(SaaS 랜딩 자체가 원래 길다) 참고 지표로 기록
  2. **광고/CTA 밀도**: i-boss.co.kr 첫 화면에만 배너 광고 4개 이상(아이보스 기업교육·장관상 5관왕·이벤트·로그인 유도 박스) + 상단 메뉴 10개 항목 동시 노출 — Nielsen #8(미학적·미니멀 디자인) 기준으로 AEOlab의 단일 히어로+단일 CTA 구조가 시각적으로 확연히 정돈되어 있음(실측 스크린샷 근거, §6.1의 추정이 아닌 확정)
  3. **가격 정보 구조**: §6.1의 코드 대조 결론(아이보스에 통합 요금제 페이지 없음)이 시각적으로도 확인됨 — i-boss 홈의 "마케팅 대행의뢰" 섹션은 개별 게시글 형태(일 최소~최대 금액이 게시글마다 다름, 예: "일 최소 10만원~최대 1,000만원")로 표준화된 가격 체계가 아님. AEOlab `/pricing`은 4단계 고정가(9,900~49,900원)+기능비교표+FAQ가 한 페이지에 즉시 노출 — **가격 투명성 축에서 AEOlab 우위 재확인(시각 근거로 격상)**
  4. **카테고리 이질성 재확인**: i-boss는 랭킹 게시판·채용광고·교육상품 크로스셀이 혼재된 포털이라 AEOlab과 1:1 벤치마크 타당성은 여전히 제한적 — 참고 자료로만 활용, "i-boss보다 낫다/못하다" 식 단정은 하지 않음
- **결론**: gap-5는 이제 **실제 스크린샷 근거로 완료됨**(§6.1의 코드/HTML 기반 차선책에서 격상). 카테고리가 달라 정량 순위를 매기진 않지만, "정보 위계 명확성"·"광고 밀도 대비 정돈도"·"가격 투명성" 3축에서 AEOlab이 구조적 우위에 있다는 판단을 시각 자료로 뒷받침함
- 스크린샷 파일: 세션 로컬 임시경로(`%TEMP%/gap5/`)에 저장, 문서에는 첨부하지 않음(민감정보 없으나 임시파일 성격)

...
>
> **2026-05-04 A안 50/50 변경**: Basic 자동 스캔에서 Gemini 50회 + ChatGPT 50회 동등 측정으로 전환. 한국 사용자 인지도가 압도적으로 높은 ChatGPT를 통계적으로 동등 처리. 점수 산식: Gemini 45점 + ChatGPT 45점 = 90점 → 100점 재배분 (`calc_multi_ai_exposure`). Full 유료 스캔은 100/100 그대로 유지.
...
### D-2. 2026 AI 탭 대응 (2026-04-27 베타 공개됨)

- **현황**: 네이버 AI 탭 2026-04-27 베타 공개 (네이버플러스 우선, 대화형 + 예약·결제 연동)
- **AEOlab 대응**: AI 탭 노출 스캐너 추가 (naver_scanner.py 확장 또는 별도 파일)
- **우선 준비**: 소개글 안 Q&A 추가·소개글 품질이 AI 탭에서도 동일하게 랭킹 요소가 될 것으로 예상
- **상태**: 전 업종 정식 확대 후 실제 노출 구조 확인 후 대응

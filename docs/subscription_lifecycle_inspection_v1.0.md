...
### 배포 직후 자기점검에서 발견·수정한 3건 (git `c17a141`, `caf31df`, `e9b9e5d`)

사용자가 "오판과 오류가 없었는지" 질문 → 구현을 아드버서리얼하게 재점검해 실제로 3건을 더 발견·수정:

1. **환불 성공 후 DB 갱신 실패 시 silent 불일치** — Toss 환불 API가 성공한 직후 `subscriptions.status` 갱신 쿼리가 실패하면(네트워크 순간오류 등) "환불은 됐는데 DB는 active로 남는" 불일치가 조용히 발생할 수 있었음(`try/except` 없이 무방비). money-moving 이벤트 직후라 다른 환불 실패 경로와 동일하게 operator alert+Slack 알림 추가.
2. **§1(7일 전 만료안내) 2일 폭 윈도우가 자체 회귀였음** — "매일 무한 반복 발송 방지"만 신경쓰다 "고정폭 윈도우도 매 구독자에게 정확히 2회 중복발송"이라는 점을 놓침. `end_at` 날짜단위 통일로 이제 `.eq()` 정확일치만으로도 안정적이라 되돌림. §2(갱신, P0)의 `.lte()` 완화는 유지 — 이건 실제로 필요한 방어(놓치면 매출 손실)이고 §1은 그렇지 않다는 점을 구분.
3. **프론트 환불자격 배너가 백엔드 권위판정과 어긋날 수 있었음** — `page.tsx`가 `is_active=true` 필터된 사업장만으로 힌트를 계산해, "스캔 이력 있는 사업장을 비활성화한 사용자"에게 실제로는 거부될 환불을 배너로 잘못 약속할 뻔함. 백엔드 `_check_refund_eligibility()`와 동일하게 전체 사업장 기준으로 재조회하도록 수정.

검증 방식: Python 구문 `ast.parse` 전수 확인, `subscription_lifecycle_job`이 실제로 스케줄러(`jobs.py:67`, 매일 01:00)에 등록돼 있는지 재확인, `count="exact"`가 `.limit(1)`과 무관하게 정확한 총계를 반환하는지 실 데이터로 실측(28건 확인), 로컬↔서버 md5 전체 재대조.

### 실 결제 전환 전 라이브 QA (2026-07-07, 실카드+실 Toss 테스트모드 결제)

사용자가 실제 aeolab.co.kr에서 신규 가입 → 실카드로 Basic 구독 → 설정 페이지 → 해지까지 전체 플로우를 라이브로 직접 실행. 이 과정에서 코드 검토만으로는 못 잡았을 실행 경로 버그를 다수 발견·즉시 수정:

1. **`webhook.py issue_billing()` — `profiles.id` 존재하지 않는 컬럼 조회로 매번 크래시 (P0, git `cb9739c`)**: `profiles` 테이블은 `user_id`가 PK이고 `id` 컬럼이 없는데 `.select("id").eq("id", user_id)`로 조회 — 토스 카드 인증(빌링키 발급)까지는 실제로 성공한 뒤 이 검증 단계에서 항상 크래시. **이 버그로 인해 이 코드 경로로는 신규 구독 결제가 단 한 건도 완료된 적이 없었을 가능성이 높음** — 기존 테스트 구독 5건이 전부 수동삽입/dev_bypass였던 이유와 정확히 부합. 실 결제 전환 직전에 라이브 QA로 잡음.
2. **`middleware.ts` — 사업장 미등록 구독자가 `/settings` 접근 시 온보딩으로 강제 리다이렉트 (P0, git `33968bf`)**: 결제는 마쳤지만 사업장 미등록(`bizCount===0 && !onboarding_done`) 구독자가 해지 버튼에 아예 도달 못 함 — 방금 구현한 7일 자동환불의 최우선 대상 사용자군을 정확히 막고 있었음. `/settings`를 온보딩 강제 리다이렉트 예외 경로로 추가.
3. **온보딩 1단계에서 pricing으로 가는 경로 부재 (미수정, 별도 트랙)**: `onboarding/page.tsx`의 요금제 선택 모달은 3단계(사업장등록→첫스캔안내→시작하기) 완료 후에만 등장 — 1단계 화면엔 pricing 링크가 전혀 없음. 실전환율에 영향 줄 수 있는 UX 이슈로 남겨둠(사용자 결정 대기).
4. **`SettingsClient.tsx` — 해지 직후 상단 "구독 중" 배지 미갱신 (git `5128a64`, `aed9dbd`)**: `router.refresh()`를 추가했으나 실측 결과 소프트 리프레시로는 반영 안 됨(수동 F5는 정상 — DB는 매번 정확했음, 순수 클라이언트 렌더링 이슈). 해지완료 배너 2.5초 노출 후 `window.location.reload()`로 최종 수정.
5. **`SettingsClient.tsx handleCardChange()` — 카드변경 팝업을 닫기만 해도 "취소되었습니다" 오류 표시 (git `5128a64`)**: `err.message.includes("PAY_PROCESS_CANCELED")`라는 잘못된 필드/값 검사 — `PayButton.tsx` 등 나머지 4곳은 전부 `err.code !== "USER_CANCEL"`로 정상 판별. 동일 패턴으로 통일.
6. **`payment/success/page.tsx` — 결제 성공 화면에서 SiteFooter가 카드 옆에 나란히 붙는 레이아웃 깨짐 (git `140b144`)**: `<main>`이 `flex items-center justify-center` 하나로 성공 카드와 `SiteFooter`를 동일 가로 flex 자식으로 취급. `flex-col` + `flex-1` 래퍼로 분리.

**사용자 추가 요청으로 신규 구현(같은 세션)**: 등록 카드 정보 표시(`card.issuerCode`/`card.number` 캡처, Toss 공식 카드사 코드표 확인 후 매핑) + 해지/환불 완료 카카오 알림톡(기존 승인된 `AEOLAB_NOTICE_01` 템플릿 재사용, 신규 심사 불필요) — git `b45f067`.

### 잔여
- §5, §6, 라이브 QA 6건 전부 배포 완료.
- **미해결 (별도 트랙)**: 온보딩 1단계 → pricing 경로 부재(#3) — 사용자 결정 대기.
- 카카오 알림톡은 전화번호 미등록 테스트 계정이라 실제 발송까지는 검증 못 함(코드 경로는 기존 4개 알림과 동일 패턴이라 신뢰도 높음).

...
> 새 대화창에서 이어가려면: `docs/blog_diagnosis_session_2026_07_18_handoff_v1.0.md 기준으로 §6-1(경쟁사 대비 패널 노출 개선)부터 진행`
...
| ③-b 경쟁사 대비 (경쟁사관리 크롤링 재사용) | `competitor_blog_comparison`, `competitor_blog_mentions` | 코드 정상, **데이터도 있음** — 단 "상세 데이터 보기" 토글 안에 숨어있어 기본 비노출 (§5-1) |

naeo.kr 대비: 콘텐츠 정성판정(LLM 기반)은 AEOlab이 우위. "AI 랭킹"(업종 내 TOP3 리더보드)은 naeo.kr에 있고 AEOlab엔 없음 — 이것도 ③-a와 동일하게 최소표본 문제라 지금 만들어도 못 채움. 반면 ③-b(경쟁사 대비)는 표본 문제가 없다 — naeo.kr에 없는 AEOlab만의 차별화 지점이 될 수 있음(§5-1).

### 5-1. "경쟁사 관리" 페이지와 중복인가 — 아니다, 단 지금 숨겨져 있다

2026-07-18 사용자가 "경쟁사 대비를 포함해달라"면서 두 가지를 판단해달라고 요청: (1) 서버 부하, (2) "경쟁사 관리" 페이지와의 중복 여부.

**서버 부하 판단**: 방식에 따라 다르다.
- **방식 A(채택, §1 참조)** — 경쟁사 관리가 이미 주간 배치로 수집해둔 `comp_keywords`/`blog_mention_count`를 재사용. DB 조회만 하므로 **추가 부하 없음**. 이미 구현됨.
- **방식 B(비채택)** — 경쟁사의 "블로그" 자체를 실시간 크롤링. `competitors` 테이블에 블로그 URL 컬럼이 없어 데이터 소스가 없고, 있어도 서버(vCPU2/RAM4GB, `PLAYWRIGHT_MAX_CONCURRENCY=1` 전역 공유)에 실제 부하가 걸림 — §2-B(경쟁사 실시간 구조비교, 영구 제외)와 본질적으로 동일한 항목.

**중복 판단**: `gap_analyzer.py`(경쟁사 관리 페이지가 쓰는 `analyze_gap`)와 `blog_analyzer.py`(`_build_competitor_comparison`)를 대조한 결과, 경쟁사 쪽 데이터는 동일(`comp_keywords`)하지만 **"내 쪽" 비교 기준이 다르다** — 경쟁사 관리는 "내 리뷰"(리뷰 유도 전략), 블로그 진단은 "내 블로그 글"(소재 전략). 기능적으로 중복이 아니다. 실제로 과거 세션에 점수·순위 비교 카드는 "같은 갭을 더 상세히 보여주므로 중복"이라 판단해 제거한 이력이 있고(`BlogClient.tsx:2330-2333` 주석), 지금 남은 두 패널(`CompetitorKeywordBlockedPanel`, `BlogMentionBenchmarkCard`)은 그 중복 검토를 이미 통과한 것들이다.

**단, 실제 문제 발견**: 이 두 패널이 페이지의 **"상세 데이터 보기" 토글 안에 기본 접힌 상태**로 숨겨져 있다(`BlogClient.tsx:2321-2324`). 사용자가 "경쟁사 대비를 포함해달라"고 재요청한 것 자체가, 이미 구현된 이 기능이 눈에 잘 안 띈다는 신호로 해석된다 — §6 우선순위 1번으로 반영.

## 6. 잔여 개선사항 (우선순위순)

1. **[신규, 최우선] 경쟁사 대비 패널(③-b) 노출 개선** — `CompetitorKeywordBlockedPanel`(경쟁사 키워드 점유 현황)·`BlogMentionBenchmarkCard`(블로그 언급 수 비교)가 현재 "상세 데이터 보기" 토글 안에 기본 접혀있음(`BlogClient.tsx:2321-2339`). 방식 A(comp_keywords 재사용, 부하 없음, §5-1) 그대로 유지하되 **기본 노출로 승격하거나 페이지 상단 가까이 배치** — 사용자가 "경쟁사 대비 포함"을 재요청한 근본 원인으로 판단됨. **아직 미착수.**
2. **`BlogScoreTrendChart` 배치 개선** — 지금 페이지 최하단(§4 참조). "내가 나아지고 있나"를 보여줄 수 있는 자기추세 축인데 우선순위가 낮게 배치돼 있음. 체크리스트 직후(상단)로 옮기는 걸 권장 — **아직 미착수**. 1번과 함께 페이지 상단부 재배치를 한 번에 설계하는 게 효율적.
3. **③-a(업종 평균 대비, citation_benchmark) 신규 개발은 지금 하지 말 것** — 코드가 정상이어도 표본 부족(전체 사업장 7곳)으로 못 채움. 구독자 20명 이상, 업종당 3곳 이상 모이는 시점에 재점검. (③-b 경쟁사 대비는 이 제약이 없음 — 1번 참조)
4. **comp_keywords 백필 자동화 확인** — 목요일 03:00 `enrich_competitor_details_job`이 이번에 수동 백필한 것과 동일하게 stale 데이터를 알아서 재동기화하는지 다음 주 목요일 이후 실측 확인 권장(현재는 코드 로직상 될 것으로 추정만 함, 실측 미확인).
5. **git push** — 이 세션과 별개 작업으로 보류 중(140+ 서버-로컬 diverge 파일, 서버 git HEAD `917b650` 이후 정체). `scripts/check_server_drift.sh`로 방향 판정 후 진행 — 사용자가 별도 요청 시에만.
6. **외부 블로그(Tistory/WordPress) LLM 품질판정** — 의도적 제외 상태(제목만 확보 가능, 본문 텍스트 없음). 크롤링 방식이 바뀌지 않는 한 재검토 불필요.

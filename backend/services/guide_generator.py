...
[D.I.A. 5요소 — 반드시 모두 적용]
D(Diversity, 다양성): LSI 연관 키워드 중 4개 이상을 본문에 자연스럽게 사용 (단순 나열 금지). **단, 예외 없이 지킬 것** — LSI 키워드가 이 사업장이 실제로 제공하는지 확인되지 않은 시설·서비스·정책·수용범위를 암시하면(예: "프라이빗룸", "30명 수용", "단체석 있음", "유아 의자 있음", "단체 예약 가능", "회식 장소", "반려견 동반 가능" 등 — 이 목록은 예시일 뿐 전부가 아님, 같은 성격의 다른 키워드도 동일하게 취급할 것) 절대 사실 단정문("~가능합니다", "~을 갖추고 있습니다", "~로 활용하기 좋습니다")으로 쓰지 말 것. 반드시 "~가능 여부는 스마트플레이스 또는 전화로 확인해 주세요"처럼 유보·확인유도 표현으로 쓰거나, 자연스러운 문장이 안 되면 해당 키워드는 생략할 것. 이 사업장의 실제 정보(주소·전화·리뷰수·평점·핵심 키워드/메뉴)에서 직접 확인되는 내용만 단정문으로 써도 된다.

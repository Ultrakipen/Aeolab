...
A(Authority, 권위): 리뷰 수·평균 평점이 제공된 경우(0이 아닌 실제 값) 반드시 그 수치를 "리뷰 000개, 평균 평점 0.0" 형식 그대로 본문에 포함할 것 — 생략 금지. 확인되지 않은 운영 연수·수상·인증은 절대 지어내지 말 것. 리뷰 수·평점이 모두 0이거나 미제공이면 이 요소는 생략.
T(Timeliness, 적시성): 마지막 문단 끝에 "[{current_year}년 {current_month}월 기준]" 자동 삽입 (필수)
O(Originality, 독창성): 차별점은 반드시 실제 제공된 핵심 키워드(메뉴·서비스명)에서만 도출할 것 (예: 특정 메뉴명 강조). 확인되지 않은 분위기·시설·가격 정책(프라이빗룸·단체석 규모·유아의자 등)을 지어내 차별점으로 제시하지 말 것.

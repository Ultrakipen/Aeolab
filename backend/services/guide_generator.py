...
[D.I.A. 5요소 — 반드시 모두 적용]
D(Diversity, 다양성): LSI 연관 키워드 중 4개 이상을 본문에 자연스럽게 사용 (단순 나열 금지). 단, LSI 키워드가 시설·수용인원·정책(예: "프라이빗룸", "30명 수용", "단체석 있음", "유아 의자 있음")처럼 이 사업장에 실제로 해당하는지 확인되지 않은 항목을 암시하면, 사실 단정문으로 쓰지 말 것 — "~가능 여부는 스마트플레이스 또는 전화로 확인해 주세요"처럼 유보 표현으로 자연스럽게 녹이거나, 자연스러운 문장이 안 되면 해당 키워드는 생략할 것.

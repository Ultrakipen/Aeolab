...
_GAP_REASONS = {
    ScanContext.LOCATION_BASED: {
        ...
        "multi_ai_exposure": lambda gap: (
            # Gemini: 구글 비즈니스 프로필 → 수주~수개월 내 개선 가능
            # ChatGPT: 학습 데이터(컷오프 2024.06) 기반 — 지금 당장 개선 방법 없음, 장기 블로그·미디어 언급 누적만 가능
            # AEOlab 스캐너는 ChatGPT API 호출 방식(학습 데이터 기반). Bing 검색은 ChatGPT 웹 브라우저 모드에서만 해당
            (
                f"Gemini·ChatGPT AI에서 가게가 검색되지 않습니다 (경쟁사 대비 -{gap:.0f}점 차이). "
                "Gemini는 구글 비즈니스 프로필 등록 후 수주~수개월 내 개선됩니다. "
                "ChatGPT는 학습 데이터(2024.06 컷오프) 기반으로 지금 당장 개선할 방법이 없으며, "
                "블로그·미디어 언급이 장기적으로 누적돼야 반영됩니다."
            )
            if gap > 0 else
            (
                "Gemini·ChatGPT AI에서 가게가 아직 검색되지 않습니다. "
                "Gemini 개선을 위해 구글 비즈니스 프로필을 먼저 등록하세요. "
                "홈페이지가 있다면 가이드 탭의 'AI 정보 코드'를 추가하면 Gemini 노출에 도움됩니다. "
                "ChatGPT는 학습 데이터 기반으로 단기 개선이 어렵습니다 (한국 소상공인 평균 노출률 1~3%)."
            )
        ),
        ...
        "blog_crank": lambda gap: (
            f"블로그 생태계 점수가 경쟁사보다 낮습니다 (경쟁사 대비 -{gap:.0f}점 차이). 네이버 블로그에 가게 소개 글을 월 1~2회 꾸준히 발행하면 점수가 서서히 올라갑니다."
            if gap > 0 else
            "블로그 발행 이력이 확인되지 않습니다. 네이버 블로그를 개설하고 가게 소개 글 1개를 올리는 것이 첫 단계입니다."
        ),
        "local_map_score": lambda gap: (
            f"지도·플레이스 노출이 경쟁사보다 부족합니다 (경쟁사 대비 -{gap:.0f}점 차이). 카카오맵 사업장 정보를 등록하고 대표 사진 3장 이상을 추가하면 지도 노출이 늘어납니다."
            if gap > 0 else
            "카카오맵 사업장 등록 여부를 확인하세요. 등록 후 영업시간·전화번호·사진을 채우면 지도 검색 노출이 시작됩니다."
        ),
        "ai_briefing_score": lambda gap: (
            f"AI 브리핑 노출 점수가 경쟁사보다 낮습니다 (경쟁사 대비 -{gap:.0f}점 차이). 소개글·사진을 완성하고 톡톡 채팅방 메뉴를 설정하면 노출 가능성이 높아집니다."
            if gap > 0 else
            "AI 브리핑에 아직 노출되지 않습니다. 소개글·사진 완성도를 높이면 반영 여부는 다음 스캔에서 확인하실 수 있습니다."
        ),
    },
    ScanContext.NON_LOCATION: {
        ...
        "multi_ai_exposure": lambda gap: (
            # Gemini: 구글 비즈니스 프로필·웹콘텐츠 → 수주~수개월 내 개선 가능
            # ChatGPT: 학습 데이터(컷오프 2024.06) 기반 — 장기 블로그·미디어 언급 누적만 반영
            (
                f"Gemini·ChatGPT AI에서 아직 인용되지 않습니다 (경쟁사 대비 -{gap:.0f}점 차이). "
                "Gemini 노출을 위해 구글 비즈니스 프로필 완성과 전문 콘텐츠 발행이 필요합니다. "
                "ChatGPT는 학습 데이터 기반으로 전문 블로그 글·인터뷰·기사가 장기적으로 누적돼야 반영됩니다."
            )
            if gap > 0 else
            (
        ...
        "blog_crank": lambda gap: (
            f"블로그 생태계 점수가 경쟁사보다 낮습니다 (경쟁사 대비 -{gap:.0f}점 차이). 전문 분야 블로그 글을 월 1~2회 꾸준히 발행하면 점수가 서서히 올라갑니다."
            if gap > 0 else
            "블로그 발행 이력이 확인되지 않습니다. 네이버 블로그나 전문 블로그에 서비스 소개 글 1개를 올리는 것이 첫 단계입니다."
        ),
        "local_map_score": lambda gap: (
            f"지도·플레이스 노출이 경쟁사보다 부족합니다 (경쟁사 대비 -{gap:.0f}점 차이). 카카오맵 사업장 정보를 등록하고 연락처·서비스 설명을 완성하면 지도 노출이 늘어납니다."
            if gap > 0 else
...
_REVIEW_CATEGORIES_BY_TYPE: dict[str, dict[str, list[str]]] = {
    # 음식점·카페·베이커리·바
    "food": {
    ...
    # 뷰티·네일
    "beauty": {
        "시술결과":   ["자연스러운", "예쁜", "발색", "지속력", "퀄리티", "완성도"],
        "서비스":     ["친절", "꼼꼼", "배려", "상담", "추천", "원장님"],
        "위생·분위기": ["깔끔", "청결", "위생", "분위기", "인테리어", "조용"],
        "지속력·관리": ["지속", "유지", "재방문", "관리", "보수", "오래"],
        "가격·가성비": ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 의료·약국·피트니스·요가
    "health": {
        "효과·결과":  ["효과", "호전", "개선", "회복", "통증", "결과"],
        "서비스":     ["친절", "설명", "꼼꼼", "상담", "의사", "선생님"],
        "시설·청결":  ["깔끔", "청결", "넓은", "쾌적", "시설", "위생"],
        "접근성":     ["주차", "접근", "위치", "교통", "가깝", "편리"],
        "가격·비용":  ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 교육·학원
    "education": {
        "수업품질":   ["이해", "설명", "실력", "향상", "효과적", "커리큘럼"],
        "강사":       ["친절", "열정", "꼼꼼", "선생님", "강사", "피드백"],
        "시설":       ["깔끔", "넓은", "쾌적", "시설", "교재", "자료"],
        "스케줄":     ["유연", "조율", "보강", "빠른", "일정", "시간"],
        "가격·가성비": ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 법률·부동산·인테리어·디자인 등 전문직
    "professional": {
        "전문성":     ["전문", "지식", "경험", "실력", "믿음", "노하우"],
        "소통·상담":  ["소통", "상담", "설명", "답변", "빠른", "친절"],
        "결과·만족":  ["만족", "결과", "완성도", "기대", "퀄리티", "성공"],
        "신뢰도":     ["신뢰", "믿음", "추천", "재의뢰", "후기", "인증"],
        "가격·비용":  ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 사진·영상
    "photo": {
        "촬영품질":   ["선명", "화질", "자연스러운", "퀄리티", "보정", "감각"],
        "서비스":     ["친절", "편안", "배려", "포즈", "안내", "작가님"],
        "공간·분위기": ["스튜디오", "넓은", "깔끔", "분위기", "배경", "조명"],
        "결과물·전달": ["당일", "원본", "보정본", "앨범", "빠른", "전달"],
        "가격·가성비": ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 숙박
    "accommodation": {
        "청결도":     ["청결", "깨끗", "위생", "깔끔", "쾌적", "냄새"],
        "시설·편의":  ["시설", "넓은", "편리", "주차", "조식", "편의"],
        "서비스":     ["친절", "응대", "체크인", "직원", "사장님", "빠른"],
        "위치":       ["위치", "접근", "교통", "관광", "가깝", "역"],
        "가격·가성비": ["가성비", "저렴", "합리적", "비싸", "가격"],
    },
    # 반려동물
    "pet": {
        "케어품질":   ["꼼꼼", "예쁜", "완성도", "털", "손질", "전문"],
        "서비스":     ["친절", "배려", "소통", "원장님", "사장님", "안심"],
        "시설·위생":  ["깔끔", "청결", "위생", "넓은", "시설", "환경"],
...
_CATEGORY_TO_REVIEW_TYPE: dict[str, str] = {
    ...
    "interior": "professional", "design": "professional",
...
_REVIEW_CATEGORIES_GENERIC: dict[str, list[str]] = {
    "품질":       ["품질", "퀄리티", "완성도", "전문", "실력", "결과"],
    "서비스":     ["친절", "서비스", "빠른", "응대", "직원", "사장님"],
    "분위기·시설": ["분위기", "깔끔", "청결", "쾌적", "넓은", "시설"],
    "위치·접근":  ["주차", "접근", "위치", "교통", "가깝", "편리"],
    "가격·가성비": ["가성비", "저렴", "합리적", "비싸", "가격"],
}
...
def analyze_review_keyword_distribution(
    ...
    review_type = _CATEGORY_TO_REVIEW_TYPE.get(category)
    REVIEW_CATEGORIES: dict[str, list[str]] = (
        _REVIEW_CATEGORIES_BY_TYPE.get(review_type, _REVIEW_CATEGORIES_GENERIC)
        if review_type
        else _REVIEW_CATEGORIES_GENERIC
    )

...
_GAP_REASONS = {
    ScanContext.LOCATION_BASED: {
        ...
        "schema_seo": lambda gap: "웹사이트 JSON-LD 미설정 — 글로벌 AI가 사업장 정보 인식 불가",
    },
    ScanContext.NON_LOCATION: {
        ...
        "schema_seo": lambda gap: "웹사이트 JSON-LD 없음 — AI가 사업장 정보를 구조적으로 파악 불가",
    },
}
...
_GROWTH_STAGES = [
    ...
    },
    {
        ...
        "this_week_action": "독립 웹사이트에 JSON-LD 구조화 데이터를 추가하거나 구글 비즈니스 프로필을 정비하세요",
...
def _build_growth_stage(total_score: float, track1_score: float | None = None) -> GrowthStage:
    """성장 단계 판정.
...
def analyze_gap(
    ...
    growth_stage = _build_growth_stage(my_total_score, track1_score=track1_score)
...
async def analyze_gap_from_db(business_id: str, supabase) -> Optional[GapAnalysis]:
    ...
    # 리뷰 발췌문 수집 — naver_result + gemini_result에서 excerpt 추출
    # cold start: keyword_coverage_rate 없으면 블로그 텍스트도 추출
    ...
    for blog in top_blogs:
        if isinstance(blog, dict):
            ...
            if text:
                review_excerpts.append(text)

    for result_key in ("naver_result", "gemini_result"):
        result_data = scan.get(result_key) or {}
        if isinstance(result_data, dict):
            # excerpt, review_keywords, mentions 등에서 텍스트 수집
            for field in ("excerpt", "context", "mention_text", "review_text"):
                val = result_data.get(field)
                if isinstance(val, str) and val:
                    review_excerpts.append(val)
            # 리스트 형식 필드 처리
            for field in ("excerpts", "reviews", "mentions"):
                items = result_data.get(field)
                if isinstance(items, list):
                    review_excerpts.extend(
                        item if isinstance(item, str) else item.get("text", "")
                        for item in items if item
                    )

    # ai_citations 테이블에서 추가 발췌문 (최근 20개)
    try:
        ...
        review_excerpts.extend(
            row["excerpt"] for row in cit_rows if row.get("excerpt")
        )
    except Exception as e:
    ...
    if not competitor_scores:
        growth_stage = _build_growth_stage(my_total, track1_score=my_track1)

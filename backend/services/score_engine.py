...
# AI 브리핑 노출 가능성 분류 — ⚠️ "플레이스형(가게 플레이스 카드 요약형)" 한정
# 네이버 AI 브리핑 5유형 중 "정보형/공식형 멀티출처(추천형)"는 업종 제한이 없으며,
# 블로그·카페·콘텐츠가 출처로 채택되면 전 업종이 노출될 수 있다
# (2026-06-29 실측 확인: "창원 웨딩 스냅 촬영 추천" → 사진/웨딩스냅 정보형 브리핑 노출).
# 따라서 아래 INACTIVE 분류는 "플레이스형 비대상"이며 "네이버 AI 브리핑 전면 불가"가 아니다.
